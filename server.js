...
// ═══ CREDENTIALS (set as environment variables in Render) ════════
const API_KEY  = process.env.TWELVEDATA_API_KEY  || '';
const TG_TOKEN = process.env.TELEGRAM_TOKEN      || '';
const TG_CHAT  = process.env.TELEGRAM_CHAT_ID    || '';

if (!API_KEY || !TG_TOKEN || !TG_CHAT) {
  console.warn('⚠️  Missing env vars. Set TWELVEDATA_API_KEY, TELEGRAM_TOKEN, TELEGRAM_CHAT_ID in Render.');
}
...
function findSwings(c){
  // 5-candle pivot — visually clear swings only
  const sH=[],sL=[];
...
function headSweptLiquidity(c,head,type){
  // Equal highs/lows (engineered liquidity) OR clear prior swing
...
function validateQMRCriteria(c,type,head,qmSwing,atr,sH,sL){
  ...
  // 2. Displacement — IMMEDIATE: first 3 candles after head
  ...
  // 4. Engineered Liquidity (stop cluster near QM zone before head)
...
function detectQMR(c){
  ...
  for(let h=sH.length-1;h>=1;h--){
    ...
    // AT LEVEL — strict: within 0.5 ATR AND 0.08% of QM level
    ...
    // OB/FVG HARD REQUIREMENT at QM level
    ...
    // SL = TRUE PROTECTED HIGH (max high from head candle to now)
    let trueHigh=head.p;
    for(let k=head.i;k<c.length;k++){if(c[k].high>trueHigh)trueHigh=c[k].high;}
    const retestSL=trueHigh+atr*0.25;

    results.push({type:'BEARISH',qmLevel:hl.p,head:head.p,cp,atr,criteria:crit,
      atLevel:true,drawOnLiquidity:null,retestSL});
    break;
  }
  ...
  for(let l=sL.length-1;l>=1;l--){
    ...
    // SL = TRUE PROTECTED LOW (min low from head candle to now)
    let trueLow=head.p;
    for(let k=head.i;k<c.length;k++){if(c[k].low<trueLow)trueLow=c[k].low;}
    const retestSL=trueLow-atr*0.25;

    results.push({type:'BULLISH',qmLevel:lh.p,head:head.p,cp,atr,criteria:crit,
      atLevel:true,drawOnLiquidity:null,retestSL});
    break;
  }
  return results;
...
async function tgCRT(id,crt,sc,wBias,sess){
  ...
  const msg=
    ...
    '\u26A0\uFE0F Confirm structure on chart before entering.\n\u2014 The Slayers Model by Rexroz';
  await tgSend(msg);
}
async function tgQMR(id,tf,qmr){
  ...
  // TP2 must always be further than TP1
...
// ═══ TRADE MANAGEMENT (uses full candle H/L for intracandle hits) ═
async function checkQMRTrades(instId,price,cHigh,cLow){
  ...
  for(let i=activeQMRTrades.length-1;i>=0;i--){
    ...
    if(!t.slFired&&(isB?lo<=t.sl:hi>=t.sl)){t.slFired=true;await tgQMRUpdate(t,'sl');activeQMRTrades.splice(i,1);continue;}
    if(!t.beFired&&(isB?price>=t.beLevel:price<=t.beLevel)){t.beFired=true;await tgQMRUpdate(t,'be');}
    if(!t.tp1Fired&&(isB?hi>=t.tp1:lo<=t.tp1)){t.tp1Fired=true;await tgQMRUpdate(t,'tp1');}
    if(!t.tp2Fired&&(isB?hi>=t.tp2:lo<=t.tp2)){t.tp2Fired=true;await tgQMRUpdate(t,'tp2');activeQMRTrades.splice(i,1);}
  }
}
...
async function runScan(manual=false){
  ...
  if(!isWeekend()){
    ...
    if(h===7&&m<30&&lastBriefing!==today){lastBriefing=today;await tgDailyBriefing(briefingData);}
  }
    ...
    try{
      ...
      if(crt&&!(isWeekend()&&inst.id!=='BTCUSD')){
        ...
        if(sc.score>=CRT_MIN){
          ...
          if(!crtSeen.has(key)){
            crtSeen.add(key);
            log(`CRT ALERT: ${inst.id} DAILY ${crt.type} Score:${sc.score}/10`);
            await tgCRT(inst.id,crt,sc,wc.bias,sess);
  ...
  // QMR scan (every other 30min scan = ~60min cycle)
      ...
      for(const tf of QMR_TFS){
        try{
          ...
          for(const qmr of qmrs){
            ...
            if(!qmrSeen.has(key)){
              qmrSeen.add(key);
              log(`QMR ALERT: ${inst.id} ${tf.toUpperCase()} ${qmr.type} Level:${qmr.qmLevel}`);
              await tgQMR(inst.id,tf==='1h'?'1H':'4H',qmr);
...
app.get('/', (req, res) => {
  ...
  res.send(`<!DOCTYPE html><html><head><title>Slayers Alert System v5</title><meta charset="UTF-8"><style>body{background:#080c10;color:#c8d8e8;font-family:monospace;padding:20px}h1{color:#26A69A;letter-spacing:1px}h2{color:#c084fc;margin-top:24px;font-size:14px;letter-spacing:1px}table{border-collapse:collapse;width:100%;margin-top:6px}td,th{border:1px solid #1e2530;padding:6px 10px;font-size:12px}th{color:#26A69A;background:#0c1218}.green{color:#4ade80}.red{color:#f87171}.dim{color:#4a6a7a}.live{color:#4ade80}</style></head><body>
<h1>SLAYERS ALERT SYSTEM v5</h1>
<p>Status: <span class="live">LIVE</span> &nbsp;|&nbsp; Uptime: ${hrs}h ${mins}m &nbsp;|&nbsp; Scans: ${scanCount} &nbsp;|&nbsp; Last: ${lastScanTime||'&mdash;'}</p>
<p class="dim">Session: ${getSess()} &nbsp;|&nbsp; Weekend: ${isWeekend()?'Yes':'No'} &nbsp;|&nbsp; CRT: ${CRT_INSTS.length} pairs &nbsp;|&nbsp; QMR: ${QMR_INSTS.length} pairs (1H + 4H)</p>
<h2>RECENT ALERTS</h2>
...
<p class="dim" style="margin-top:24px">The Slayers Model by Rex Roz &nbsp;|&nbsp; v5 final</p>
</body></html>`);
});

// ═══ START ════════════════════════════════════════════════════════
app.listen(PORT, () => log(`Status page running on port ${PORT}`));
log('Slayers Alert System v5 starting...');
runScan(true).then(() => {
  setInterval(() => runScan(false), CHECK_MS);
  log(`Scan interval set: every ${CHECK_MS/60000} minutes`);
});
