...
async function fetchNewsFeed(){
  try{
    ...
    const feeds=[
      ...
      {url:'https://www.cnbc.com/id/100003114/device/rss/rss.html',cat:'Economy',src:'CNBC'},
      {url:'https://feeds.content.dowjones.io/public/rss/mw_topstories',cat:'Economy',src:'MarketWatch'},
    ];
    const parseRSS=function(xml){
      const items=[];
      ...
      while((m=itemRegex.exec(xml))!==null){
        ...
        if(title)items.push({title,link,summary:cleanDesc,source:src,time:pubDate});
      }
      return items;
    };
    let allItems=[];
    ...
    newsFeedCache=allItems.slice(0,50);
    log('News feed: '+newsFeedCache.length+' articles');
  }catch(e){
...
async function runScan(manual=false){
  ...
  const briefingData={};
  if(Date.now()-lastNewsFetch>15*60*1000){await fetchNewsEvents();await fetchNewsFeed();}
...
loadState().then(()=>{
  ...
  fetchNewsFeed().then(()=>{});runScan(true).then(()=>{setInterval(()=>runScan(false),CHECK_MS);log(`Scanning every ${CHECK_MS/60000} minutes`);});
});
