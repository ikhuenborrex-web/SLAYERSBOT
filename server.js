...
async function checkQMRTrades(instId,price,cHigh,cLow){
  ...
  for(let i=activeQMRTrades.length-1;i>=0;i--){
    ...
    if(t.tp1Fired&&!t.slFired&&(isB?hi>=t.tp2:lo<=t.tp2)){
      ...
      delete trackedTrades[t.sigId];activeQMRTrades.splice(i,1);saveState();continue;
    }
...
function markFeedOutcome(sigId, outcome){
  const entry=appSignalFeed.find(s=>s.id===sigId);
        ...
        try{
          ...
          for(const qmr of qmrs){
            ...
            if(!qmrSeen.has(key)){
              ...
              const earlyKey2=inst.id+'-'+qmr.type+'-'+qmr.qmLevel.toFixed(3);
              const earlyData2=earlyEntryCache[earlyKey2];
              ...
              if(earlyData2){
                delete earlyEntryCache[earlyKey2];
                const eSigId='qmr-e-'+(++chartCounter)+'-'+Date.now();
                activeQMRTrades.push({sigId:eSigId,instId:inst.id,instName:instObj.name,tf:qmrTF,type:qmr.type,qmLevel:earlyData2.entryPrice,sl:earlyData2.sl,tp1:earlyData2.tp1,tp2:earlyData2.tp2,beLevel:isB2?earlyData2.entryPrice+Math.abs(earlyData2.entryPrice-earlyData2.sl)*1.3:earlyData2.entryPrice-Math.abs(earlyData2.entryPrice-earlyData2.sl)*1.3,origSL:earlyData2.sl,isElite:qmr.criteria.score>=4,eliteFactors:qmr.criteria.factors,dailyPOI:qmr.dailyPOI||null,dec:instObj.dec,beFired:false,tp1Fired:false,tp2Fired:false,slFired:false,trailActive:false,openTime:Date.now(),isEarly:true});
              }
              ...
              const earlyKey=inst.id+'-'+qmr.type+'-'+qmr.qmLevel.toFixed(3);
              const earlyData=earlyEntryCache[earlyKey];
              appSignalFeed.unshift({
                ...
                earlyEntry:earlyData?earlyData.entryPrice:null,
                earlySl:earlyData?earlyData.sl:null,
                earlyTp1:earlyData?earlyData.tp1:null,
                earlyTp2:earlyData?earlyData.tp2:null,
                tier:qmr.criteria.score>=4?'ELITE':'VALID',
                criteria:qmr.criteria.factors,score:qmr.criteria.score,
                dailyPOI:qmr.dailyPOI||null,rsiDivergence:qmr.rsiDivergence||null,counterTrend:qmr.counterTrend||false,htfBias:htfBias,
          ...
          if(tf==='1h'){
            ...
            for(const eQ of earlyQmrs){
              ...
              const eTP2=eQ.tp2;
              log('QMR EARLY: '+inst.id+' 1H '+eQ.type+' sweep@'+eQ.entryPrice.toFixed(instObj?instObj.dec:5)+' QM@'+eQ.qmLevel.toFixed(instObj?instObj.dec:5)+' RR:'+eQ.rr.toFixed(1));
              qmrSeen.add(eKey);
              await tgQMRPreAlert(inst.id,'1H',eQ,htfBias,eQ.entryPrice,eQ.wickRatio,eQ.retestSL,eQ.tp1,eTP2);
              // Add aggressive entry to app feed + active trades immediately
              const earlySigId='qmr-e-'+(++chartCounter)+'-'+Date.now();
              const eIsB=eQ.type==='BULLISH';
              activeQMRTrades.push({sigId:earlySigId,instId:inst.id,instName:instObj.name,tf:'1H',type:eQ.type,qmLevel:eQ.entryPrice,sl:eQ.retestSL,tp1:eQ.tp1,tp2:eTP2,beLevel:eIsB?eQ.entryPrice+Math.abs(eQ.entryPrice-eQ.retestSL)*1.3:eQ.entryPrice-Math.abs(eQ.entryPrice-eQ.retestSL)*1.3,origSL:eQ.retestSL,isElite:eQ.criteria.score>=4,eliteFactors:eQ.criteria.factors,dailyPOI:null,dec:instObj.dec,beFired:false,tp1Fired:false,tp2Fired:false,slFired:false,trailActive:false,openTime:Date.now(),isEarly:true});
              const eChartFile=await tgSendChart(inst.id,'1h',[
                ...
                {price:eTP2,text:'TP2',color:'rgb(245,166,35)'}
              ],'\u26A1 EARLY '+(eQ.type==='BULLISH'?'BUY':'SELL')+' '+inst.id+' 1H QMR\nAggressive: '+eQ.entryPrice.toFixed(instObj.dec)+' | SL: '+eQ.retestSL.toFixed(instObj.dec)+' | TP2: '+eTP2.toFixed(instObj.dec)+'\n\u2014 The Slayers Model by Rexroz',earlySigId);
              appSignalFeed.unshift({
                id:earlySigId,system:'QMR',pair:inst.id,tf:'1H',type:eQ.type,
                zone:eIsB?'DISCOUNT':'PREMIUM',
                entry:eQ.entryPrice,refinedEntry:null,refinedSource:null,
                sl:eQ.retestSL,tp1:eQ.tp1,tp2:eTP2,
                earlyEntry:null,earlySl:null,earlyTp1:null,earlyTp2:null,
                ...
                isEarly:true,conservativePending:true,conservativeQmLevel:eQ.qmLevel
              });
              // Cache early data so the conservative signal can reference it later
              earlyEntryCache[inst.id+'-'+eQ.type+'-'+eQ.qmLevel.toFixed(3)]={entryPrice:eQ.entryPrice,sl:eQ.retestSL,tp1:eQ.tp1,tp2:eTP2,wickRatio:eQ.wickRatio,head:eQ.head,criteria:eQ.criteria};
              log('QMR EARLY added to app: '+inst.id+' 1H '+eQ.type+' @ '+eQ.entryPrice.toFixed(instObj.dec));
            }
          }
          await sleep(DELAY_MS);
...
app.get('/api/signals',(req,res)=>{
  ...
  const out=appSignalFeed.slice(0,limit).map(s=>({...s,chartUrl:s.chartFile?'/api/chart/'+s.chartFile:null,isTracked:!!(trackedTrades[s.id]&&trackedTrades[s.id].includes(myCode))}));
