...
const PAIR_SESSIONS = {
  ...
  AUDUSD:{s:0, e:16},GBPJPY:{s:0, e:22},GBPAUD:{s:0, e:16},
  USDCAD:{s:13,e:22},XAUUSD:{s:7, e:22},US30:  {s:13,e:22},
  SP500: {s:13,e:22},BTCUSD:{s:0, e:24},
};
const PAIR_CURRENCIES = {
  EURUSD:['EUR','USD'],GBPUSD:['GBP','USD'],USDJPY:['USD','JPY'],
  AUDUSD:['AUD','USD'],GBPJPY:['GBP','JPY'],GBPAUD:['GBP','AUD'],
  USDCAD:['USD','CAD'],XAUUSD:['XAU','USD'],US30:['USD'],
  SP500:['USD'],BTCUSD:['BTC'],
};
...
async function tgQMRUpdate(trade,level){
  ...
  const icons={be:'\u26A1 MOVE TO BREAKEVEN',tp1:'\u2705 TP1 HIT!',tp2:'\uD83D\uDCB0 FULL TARGET HIT!',sl:'\uD83D\uDEAB STOP LOSS HIT',be_close:'\u2705 TRADE CLOSED \u2014 TP1 SECURED'};
  const msgs={
    be:'Trade moved 1:1 in your favour.\nMove SL to entry: '+trade.qmLevel.toFixed(p)+'\nTrade is now risk-free.',
    tp1:'\uD83C\uDFAF Draw on Liquidity: '+trade.tp1.toFixed(p)+' reached.\nClose 50% of your position now.\nMove SL to entry and let the rest run to TP2.',
    ...
    be_close:'TP1 was secured. Price returned to entry.\nRemainder closed at breakeven.\nTP1 profit is yours. \u2705 Recorded as WIN.',
  };
...
async function tgWeeklySummary(){
  ...
  const withDur=tradeHistory.filter(t=>t.duration!=null);
  const avgDur=withDur.length?Math.round(withDur.reduce((a,t)=>a+t.duration,0)/withDur.length):null;
...
// Hierarchy: Weekly Bias > Daily CRT > 4H QMR > 1H QMR
// Higher timeframe always wins and cancels lower TF opposite trades

function tfWeight(tf){
  // Returns numeric weight — higher = more authority
  if(tf==='DAILY')return 3;
  if(tf==='4H')return 2;
  if(tf==='1H')return 1;
  return 0;
}

async function tgTradeInvalidated(trade,reason,weeklyBias){
  const isB=trade.type==='BULLISH';
  const biasLine=weeklyBias&&weeklyBias!=='NEUTRAL'?
    '\n\uD83D\uDCC5 Weekly Bias: '+weeklyBias+' \u2014 market aligned with new direction':'';
  const msg=
    '\u26A0\uFE0F TRADE INVALIDATED\n'+'='.repeat(28)+'\n'+
    '\uD83D\uDCCA '+trade.instName+' | '+(isB?'BUY':'SELL')+' QMR '+trade.tf+'\n\n'+
    '\uD83D\uDD34 Reason: '+reason+biasLine+'\n\n'+
    (isB?'Close any open BUY positions on '+trade.instName+'.':
         'Close any open SELL positions on '+trade.instName+'.')+'\n\n'+
    '\u2014 The Slayers Model by Rexroz';
  await tgSend(msg);
}

// Returns true if new signal should be BLOCKED (higher TF opposite trade active)
async function resolveConflicts(instId,newType,newTf,source,weeklyBias){
  const newWeight=tfWeight(newTf);
  let blocked=false;
  for(let i=activeQMRTrades.length-1;i>=0;i--){
    const t=activeQMRTrades[i];
    if(t.instId!==instId)continue;  // different pair — ignore
    if(t.type===newType)continue;   // same direction — no conflict
    const existingWeight=tfWeight(t.tf);
    if(newWeight>=existingWeight){
      // New signal is equal or higher TF — cancel the existing conflicting trade
      const reason=source==='CRT'?
        'Daily CRT confirmed '+newType+' \u2014 institutional structure shifted on Daily timeframe':
        newTf+' QMR '+newType+' confirmed \u2014 '+newTf+' structure overrides '+t.tf;
      log(`INVALIDATING: ${t.instName} ${t.type} ${t.tf} by new ${source} ${newTf} ${newType}`);
      await tgTradeInvalidated(t,reason,weeklyBias);
      tradeHistory.push({instId:t.instId,type:t.type,tf:t.tf,outcome:'INVALIDATED',time:new Date().toISOString()});
      activeQMRTrades.splice(i,1);
    } else {
      // Existing trade is HIGHER TF opposite — block the new lower TF signal
      log(`BLOCKED: New ${instId} ${newTf} ${newType} — existing ${t.tf} ${t.type} is higher authority`);
      blocked=true;
    }
  }
  return blocked;
...
async function checkQMRTrades(instId,price,cHigh,cLow){
  ...
  for(let i=activeQMRTrades.length-1;i>=0;i--){
    ...
    if(!t.slFired&&(isB?lo<=t.sl:hi>=t.sl)){
      ...
      if(t.tp1Fired){
        // TP1 was already hit — this is BE or partial win, not a loss
        await tgQMRUpdate(t,'be_close');
        tradeHistory.push({instId:t.instId,type:t.type,tf:t.tf,outcome:'WIN',time:new Date().toISOString(),duration});
        lossStreak=0;winStreak++;if(winStreak===3)await tgSend('\uD83D\uDD25 3 wins in a row. System is performing. Stay disciplined and trust the process.\n\u2014 The Slayers Model by Rexroz');
      } else {
        // TP1 never hit — genuine stop loss
        await tgQMRUpdate(t,'sl');
        tradeHistory.push({instId:t.instId,type:t.type,tf:t.tf,outcome:'SL',time:new Date().toISOString(),duration});
        winStreak=0;lossStreak++;if(lossStreak>=2)await tgSend('\u26A0\uFE0F 2 consecutive losses. Review your risk. Reduce position size if needed.\n\u2014 The Slayers Model by Rexroz');
      }
      activeQMRTrades.splice(i,1);continue;
    }
    if(!t.beFired&&(isB?price>=t.beLevel:price<=t.beLevel)){
      t.beFired=true;
      // Update internal SL to entry — trade is now risk-free
      t.sl=t.qmLevel;
      t.beTime=Date.now();
    ...
    if(!t.tp1Fired&&(isB?hi>=t.tp1:lo<=t.tp1)){
      t.tp1Fired=true;
      t.tp1Time=Date.now();
      t.openTime=t.openTime||t.tp1Time;
      await tgQMRUpdate(t,'tp1');
      tradeHistory.push({instId:t.instId,type:t.type,tf:t.tf,outcome:'TP1',time:new Date().toISOString(),duration:t.openTime?Math.round((Date.now()-t.openTime)/60000):null});
    }
    if(!t.tp2Fired&&(isB?hi>=t.tp2:lo<=t.tp2)){
      ...
      const dur2=t.openTime?Math.round((Date.now()-t.openTime)/60000):null;
      if(r){r.outcome='TP2';r.duration=dur2;}else tradeHistory.push({instId:t.instId,type:t.type,tf:t.tf,outcome:'TP2',time:new Date().toISOString(),duration:dur2});
      lossStreak=0;winStreak++;if(winStreak===3)await tgSend('\uD83D\uDD25 3 wins in a row. System is performing. Stay disciplined and trust the process.\n\u2014 The Slayers Model by Rexroz');
      activeQMRTrades.splice(i,1);
    }
  }
      ...
      if(crt&&!(isWeekend()&&inst.id!=='BTCUSD')){
        ...
        if(sc.score>=CRT_MIN){
          ...
          if(!crtSeen.has(key)){
            ...
            // CRT is highest intraday authority — cancel any opposite QMR trades
            await resolveConflicts(inst.id,crt.type,'DAILY','CRT',wc.bias);
      ...
      for(const tf of QMR_TFS){
        try{
          ...
          for(const qmr of qmrs){
            ...
            if(!qmrSeen.has(key)){
              const qmrTF=tf==='1h'?'1H':'4H';
              const htfBias=weeklyCache[inst.id]?.bias||'NEUTRAL';
              // Check conflicts BEFORE firing — higher TF opposite trade blocks this signal
              const isBlocked=await resolveConflicts(inst.id,qmr.type,qmrTF,'QMR',htfBias);
              if(isBlocked){
                log(`QMR BLOCKED by higher TF: ${inst.id} ${qmrTF} ${qmr.type}`);
                continue;
              }
              qmrSeen.add(key);
              const sessWarn=!isPairInSession(inst.id);
              log(`QMR: ${inst.id} ${qmrTF} ${qmr.type} ${qmr.qmLevel} HTF:${htfBias}`);
              await tgQMR(inst.id,qmrTF,qmr,htfBias,sessWarn);
