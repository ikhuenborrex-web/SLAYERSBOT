...
function saveState(){
  ...
  saveTimer=setTimeout(async()=>{
    const state={
      ...
      qmrSeen:[...qmrSeen],crtSeen:[...crtSeen],appSignalFeed,lastBriefingSnapshot,lastBriefingTime,pushSubscriptions,memberCodes,trackedTrades,memberStats,
      savedAt:Date.now()
    };
...
async function fetchNewsEvents(){
  try{
    ...
    const sources=[
      {name:'investing',url:'https://economic-calendar.tradingview.com/events?from='+new Date().toISOString().slice(0,10)+'T00:00:00.000Z&to='+new Date(Date.now()+7*864e5).toISOString().slice(0,10)+'T00:00:00.000Z',map:j=>{const arr=(j&&j.result)||[];return arr.filter(e=>e.importance>=1).map(e=>({impact:e.importance===3?'High':e.importance===2?'Medium':'Low',country:(e.currency||e.country||'').toUpperCase(),title:e.title,date:e.date,description:e.description||'',forecast:e.forecast||null,previous:e.previous||null,actual:e.actual||null}));}},
...
function isLevelAlreadySeen(instId,type,qmLevel){
  for(const key of qmrSeen){
    ...
    var parts=key.split('-');
    if(parts.length>=4){
      var ts=parseFloat(parts[parts.length-2]);
      if(Date.now()-ts>48*60*60*1000)continue;
      var level=parseFloat(parts[parts.length-1]);
...
async function runScan(manual=false){
      ...
      for(const tf of QMR_TFS){
        try{
          ...
          for(const qmr of qmrs){
            ...
            const key=inst.id+'-'+qmr.type+'-'+Date.now()+'-'+qmr.qmLevel.toFixed(3);
            if(!qmrSeen.has(key)){
  ...
  if(qmrSeen.size){
    var now=Date.now(),qmrArr=[...qmrSeen];
    qmrArr=qmrArr.filter(function(k){var parts=k.split('-');if(parts.length<4)return false;var ts=parseFloat(parts[parts.length-2]);return now-ts<=72*60*60*1000;});
    if(qmrArr.length>50)qmrArr=qmrArr.slice(-50);
    qmrSeen=new Set(qmrArr);
  }
  if(crtSeen.size>400)crtSeen=new Set([...crtSeen].slice(-200));for(const k in recentQMRFires)if(Date.now()-recentQMRFires[k]>24*60*60*1000)delete recentQMRFires[k];scanCount++;lastScanTime=new Date().toISOString();saveState();
...
app.get('/api/confluence',(req,res)=>{
  ...
  for(const inst of QMR_INSTS){
    ...
    const recentCRT=recentCRTAlerts.find(a=>a.id===inst.id&&Date.now()-a.ts<24*60*60*1000);
    ...
    let signalDir='NONE';
    if(recentCRT)signalDir=recentCRT.type;
    else if(has4H)signalDir=qmr4H.qmr.type;
    else signalDir='NONE';
    ...
    if(recentCRT)factors.push('CRT: '+recentCRT.type+' '+recentCRT.score+'/10');
    ...
    pairs.push({
      ...
      signalDir,signalTier:recentCRT?recentCRT.score>=6?'ELITE':'VALID':null,
      hasCRT:!!recentCRT,has4HQMR:!!has4H,
...
loadState().then(()=>{
  ...
  setInterval(function(){fetchNewsFeed();},10*60*1000);
