...
async function fetchNewsEvents(){
  try{
    ...
    const sources=[
      {name:'investing',url:'https://economic-calendar.tradingview.com/events?from='+new Date().toISOString().slice(0,10)+'T00:00:00.000Z&to='+new Date(Date.now()+7*864e5).toISOString().slice(0,10)+'T00:00:00.000Z&countries=US,GB,EU,JP,CA,AU,NZ',map:j=>{const arr=(j&&j.result)||[];return arr.filter(e=>e.importance>=1).map(e=>({impact:e.importance===3?'High':e.importance===2?'Medium':'Low',country:(e.currency||e.country||'').toUpperCase(),title:e.title,date:e.date,description:e.description||'',forecast:e.forecast||null,previous:e.previous||null,actual:e.actual||null}));}},
...
// Auto-create journal entries for all members tracking a closed trade
function autoJournalEntry(t,outcome,rMultiple,durationMin){
  const codes=trackedTrades[t.sigId];
  if(!codes||!codes.length)return;
  ...
  // Generate review flags based on trade outcome
  const flags=[];
