...
const CRT_MIN=3,QMR_MIN=3,WEEKLY_EVERY=6,LON_S=7,LON_E=16,NY_S=13,NY_E=22;

const PAIR_SESSIONS={
  EURUSD:{s:7,e:22},GBPUSD:{s:7,e:22},USDJPY:{s:0,e:22},AUDUSD:{s:0,e:16},
  GBPJPY:{s:0,e:22},USDCAD:{s:13,e:22},XAUUSD:{s:7,e:22},BTCUSD:{s:0,e:24},
...
const PAIR_CURRENCIES={
  ...
  GBPJPY:['GBP','JPY'],USDCAD:['USD','CAD'],XAUUSD:['XAU','USD'],BTCUSD:['BTC'],
...
let winStreak=0,lossStreak=0,qmr4HCache={},pdhlSeen=new Set();
...
async function fetchNewsEvents(){
  try{const res=await fetch('https://nfs.faireconomy.media/ff_calendar_thisweek.json');const data=await res.json();newsCache=Array.isArray(data)?data.filter(e=>e.impact==='High'):[];lastNewsFetch=Date.now();log(`News: ${newsCache.length} events`);}
...
async function tgDailyBriefing(scanData){const now=new Date(),days=['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'],months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'],dateStr=days[now.getUTCDay()]+' '+now.getUTCDate()+' '+months[now.getUTCMonth()]+' '+now.getUTCFullYear();let msg='\uD83C\uDF05 SLAYERS DAILY BRIEFING\n'+dateStr+' | 07:00 UTC\n'+'='.repeat(28)+'\n\n';for(const inst of CRT_INSTS){const d=scanData[inst.id];if(!d)continue;const bias=weeklyCache[inst.id]?.bias||'NEUTRAL',bI=bias==='BULLISH'?'\uD83D\uDFE2':bias==='BEARISH'?'\uD83D\uDD34':'\uD83D\uDFE1';msg+='\uD83D\uDCCA '+inst.name+' - '+d.price+'\n'+bI+' '+bias+'\n\n';}msg+='\uD83D\uDCC5 London: 07:00-16:00 UTC | New York: 13:00-22:00 UTC\n\n\u26A0\uFE0F Wait for confirmation before entering.\n\u2014 The Slayers Model by Rexroz';await tgSend(msg);dailyAlertLog=[];}
...
async function tgEODSummary(){if(!dailyAlertLog.length)return;const now=new Date(),days=['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];let msg='\uD83D\uDCCA SLAYERS END OF DAY\n'+days[now.getUTCDay()]+' '+now.getUTCDate()+' | 22:00 UTC\n'+'='.repeat(28)+'\n\nSignals today: '+dailyAlertLog.length+'\n';dailyAlertLog.filter(a=>a.type==='CRT').forEach(a=>msg+=(a.dir==='BULLISH'?'\uD83D\uDFE2':'\uD83D\uDD34')+' '+a.id+' CRT - '+a.score+'/10\n');dailyAlertLog.filter(a=>a.type==='QMR').forEach(a=>msg+=(a.dir==='BULLISH'?'\uD83D\uDFE2':'\uD83D\uDD34')+' '+a.id+' '+a.tf+' QMR - '+a.score+'/4\n');msg+='\n\uD83D\uDD14 Review your charts. Manage open positions.\n\u2014 The Slayers Model by Rexroz';await tgSend(msg);}
async function tgWeeklySummary(){const total=tradeHistory.length;if(!total){await tgSend('\uD83D\uDCCA WEEKLY SUMMARY\nNo completed trades this week.\n\u2014 The Slayers Model by Rexroz');return;}const tp=tradeHistory.filter(t=>t.outcome==='TP1'||t.outcome==='TP2'||t.outcome==='WIN').length,sl=tradeHistory.filter(t=>t.outcome==='SL').length,wr=Math.round((tp/total)*100),bar='\u2588'.repeat(Math.round(wr/10))+'\u2591'.repeat(10-Math.round(wr/10)),now=new Date(),months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'],withDur=tradeHistory.filter(t=>t.duration!=null),avgDur=withDur.length?Math.round(withDur.reduce((a,t)=>a+t.duration,0)/withDur.length):null;let msg='\uD83D\uDCCA SLAYERS WEEKLY PERFORMANCE\nWeek ending '+now.getUTCDate()+' '+months[now.getUTCMonth()]+'\n'+'='.repeat(28)+'\n\n\uD83D\uDD25 Total: '+total+' | \u2705 TP: '+tp+' | \uD83D\uDEAB SL: '+sl+'\n\nWin Rate: '+bar+' '+wr+'%\n';if(avgDur)msg+='Avg trade duration: '+(avgDur>=60?Math.floor(avgDur/60)+'h '+(avgDur%60)+'m':avgDur+'m')+'\n';msg+='\n'+(wr>=60?'\uD83D\uDD25 Excellent week.':wr>=45?'\uD83D\uDFE1 Solid week. Stay disciplined.':'\u26A0\uFE0F Tough week. Trust the process.')+'\n\n\u2014 The Slayers Model by Rexroz';await tgSend(msg);tradeHistory=[];}

// PDH/PDL Sweep Alert
async function checkPDHLSweep(inst,c,cp,atr){
  const prevDay=getPD(c);if(!prevDay)return;
  const keyH=inst.id+'-PDH-'+prevDay.high.toFixed(inst.dec),keyL=inst.id+'-PDL-'+prevDay.low.toFixed(inst.dec);
  if(!pdhlSeen.has(keyH)&&cp>=prevDay.high*(1-0.001)&&cp<=prevDay.high*(1+0.003)){pdhlSeen.add(keyH);setTimeout(()=>pdhlSeen.delete(keyH),4*60*60*1000);log(`PDH SWEEP: ${inst.id}`);await tgSend('\uD83C\uDFAF PDH SWEEP ALERT\n'+'='.repeat(28)+'\n\uD83D\uDCCA '+inst.name+' approaching Previous Day High\n\n\uD83D\uDD34 PDH Level: '+prevDay.high.toFixed(inst.dec)+'\n\uD83D\uDCB0 Buy side liquidity sitting above this level\n\n\u26A0\uFE0F Watch for sweep + rejection. Potential sell setup forming.\n\u2014 The Slayers Model by Rexroz');}
  if(!pdhlSeen.has(keyL)&&cp<=prevDay.low*(1+0.001)&&cp>=prevDay.low*(1-0.003)){pdhlSeen.add(keyL);setTimeout(()=>pdhlSeen.delete(keyL),4*60*60*1000);log(`PDL SWEEP: ${inst.id}`);await tgSend('\uD83C\uDFAF PDL SWEEP ALERT\n'+'='.repeat(28)+'\n\uD83D\uDCCA '+inst.name+' approaching Previous Day Low\n\n\uD83D\uDFE2 PDL Level: '+prevDay.low.toFixed(inst.dec)+'\n\uD83D\uDCB0 Sell side liquidity sitting below this level\n\n\u26A0\uFE0F Watch for sweep + rejection. Potential buy setup forming.\n\u2014 The Slayers Model by Rexroz');}
}
...
async function checkQMRTrades(instId,price,cHigh,cLow){
  ...
  for(let i=activeQMRTrades.length-1;i>=0;i--){
    ...
    if(!t.slFired&&(isB?lo<=t.sl:hi>=t.sl)){
      ...
      if(t.tp1Fired){await tgQMRUpdate(t,'be_close');tradeHistory.push({instId:t.instId,type:t.type,tf:t.tf,outcome:'WIN',time:new Date().toISOString(),duration});lossStreak=0;winStreak++;if(winStreak===3)await tgSend('\uD83D\uDD25 3 wins in a row. System is performing. Stay disciplined.\n\u2014 The Slayers Model by Rexroz');}
      else{await tgQMRUpdate(t,'sl');tradeHistory.push({instId:t.instId,type:t.type,tf:t.tf,outcome:'SL',time:new Date().toISOString(),duration});winStreak=0;lossStreak++;if(lossStreak>=2)await tgSend('\u26A0\uFE0F 2 consecutive losses. Review your risk. Reduce position size if needed.\n\u2014 The Slayers Model by Rexroz');}
    ...
    if(!t.tp1Fired&&(isB?hi>=t.tp1:lo<=t.tp1)){t.tp1Fired=true;t.tp1Time=Date.now();t.openTime=t.openTime||t.tp1Time;await tgQMRUpdate(t,'tp1');tradeHistory.push({instId:t.instId,type:t.type,tf:t.tf,outcome:'TP1',time:new Date().toISOString(),duration:t.openTime?Math.round((Date.now()-t.openTime)/60000):null});}
    if(!t.tp2Fired&&(isB?hi>=t.tp2:lo<=t.tp2)){t.tp2Fired=true;await tgQMRUpdate(t,'tp2');const dur2=t.openTime?Math.round((Date.now()-t.openTime)/60000):null;const r=tradeHistory.findLast(x=>x.instId===t.instId&&x.outcome==='TP1');if(r){r.outcome='TP2';r.duration=dur2;}else tradeHistory.push({instId:t.instId,type:t.type,tf:t.tf,outcome:'TP2',time:new Date().toISOString(),duration:dur2});lossStreak=0;winStreak++;if(winStreak===3)await tgSend('\uD83D\uDD25 3 wins in a row. System is performing. Stay disciplined.\n\u2014 The Slayers Model by Rexroz');activeQMRTrades.splice(i,1);}
  }
}
...
async function runScan(manual=false){
  ...
  if(!isWeekend()&&h===7&&m<30&&lastBriefing!==today){lastBriefing=today;await tgDailyBriefing(briefingData);await tgMorningMessage();}
    ...
    for(const inst of QMR_INSTS){
      for(const tf of QMR_TFS){
        try{
          ...
          if(tf==='1h')await checkPDHLSweep(inst,c,lastC.close,calcATR(c,14));
          ...
          for(const qmr of qmrs){
            ...
            if(!qmrSeen.has(key)){
              ...
              log(`QMR: ${inst.id} ${qmrTF} ${qmr.type} ${qmr.qmLevel} HTF:${htfBias}${multiTF?' [MULTI-TF]':''}`);
              if(multiTF){await tgMultiTFConfluence(inst.id,qmr,cached4H.qmr);}
              else{await tgQMR(inst.id,qmrTF,qmr,htfBias,sessWarn);}
              ...
              activeQMRTrades.push({instId:inst.id,instName:instObj.name,tf:qmrTF,type:qmr.type,qmLevel:entry2,sl:sl2,tp1:tp1_q,tp2:tp2_2,beLevel:isB2?entry2+slDist2:entry2-slDist2,dec:instObj.dec,beFired:false,tp1Fired:false,tp2Fired:false,slFired:false,openTime:Date.now()});
...
// Dashboard
app.get('/',(req,res)=>{
  const up=process.uptime(),hrs=Math.floor(up/3600),mins=Math.floor((up%3600)/60),wr=tradeHistory.length?Math.round((tradeHistory.filter(t=>t.outcome!=='SL'&&t.outcome!=='INVALIDATED').length/tradeHistory.length)*100):0;
...
<p class="dim" style="margin-top:20px">The Slayers Model by Rex Roz | v7.2</p>
</body></html>`);
});

app.listen(PORT,()=>log(`Port ${PORT}`));
log('Slayers Alert System v7.2 starting...');
