        ...
        try{
          ...
          for(const qmr of qmrs){
            ...
            if(!qmrSeen.has(key)){
              ...
              if(earlyData)delete earlyEntryCache[earlyKey];
          ...
          if(tf==='1h'){
            ...
            for(const eQ of earlyQmrs){
              ...
              const earlyChart=await tgSendChart(inst.id,'1h',[
                {price:eQ.entryPrice,text:'ENTRY',color:'rgb(38,166,154)'},
                {price:eQ.retestSL,text:'SL',color:'rgb(244,67,54)'},
                {price:eTP2,text:'TP2',color:'rgb(245,166,35)'}
              ],'\u26A1 EARLY '+(eQ.type==='BULLISH'?'BUY + SL='+eQ.retestSL.toFixed(instObj?instObj.dec:5)+' + TP2='+eTP2.toFixed(instObj?instObj.dec:5):'SELL + SL='+eQ.retestSL.toFixed(instObj?instObj.dec:5)+' + TP2='+eTP2.toFixed(instObj?instObj.dec:5))+'\n\u2014 The Slayers Model by Rexroz');
            }
          }
          await sleep(DELAY_MS);
