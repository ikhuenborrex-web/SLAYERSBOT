...
loadState().then(()=>{
  ...
  // Clear trackedTrades entries for trades that are no longer active
  const activeSet=new Set(activeQMRTrades.map(t=>t.sigId));
  let cleaned=0;
  for(const sigId in trackedTrades){if(!activeSet.has(sigId)){delete trackedTrades[sigId];cleaned++;}}
