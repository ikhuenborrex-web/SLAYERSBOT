...
function saveState(){
  ...
  saveTimer=setTimeout(async()=>{
    const state={
      ...
      qmrSeen:[...qmrSeen],crtSeen:[...crtSeen],appSignalFeed,lastBriefingSnapshot,lastBriefingTime,pushSubscriptions,memberCodes,trackedTrades,
      savedAt:Date.now()
    };
...
async function tgWeeklySummary(){const total=tradeHistory.length;if(!total){await tgSend('\uD83D\uDCCA WEEKLY SUMMARY\nNo completed trades this week.\n\u2014 The Slayers Model by Rexroz');return;}const tp=tradeHistory.filter(t=>t.outcome==='TP1'||t.outcome==='TP2'||t.outcome==='WIN').length,sl=tradeHistory.filter(t=>t.outcome==='SL').length,be=tradeHistory.filter(t=>t.outcome==='BE').length,wr=(tp+sl)?Math.round((tp/(tp+sl))*100):0,bar='\u2588'.repeat(Math.round(wr/10))+'\u2591'.repeat(10-Math.round(wr/10)),now=new Date(),months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'],withDur=tradeHistory.filter(t=>t.duration!=null),avgDur=withDur.length?Math.round(withDur.reduce((a,t)=>a+t.duration,0)/withDur.length):null;let msg='\uD83D\uDCCA SLAYERS WEEKLY PERFORMANCE\nWeek ending '+now.getUTCDate()+' '+months[now.getUTCMonth()]+'\n'+'='.repeat(28)+'\n\n\uD83D\uDD25 Total: '+total+' | \u2705 TP: '+tp+' | \uD83D\uDEAB SL: '+sl+' | \u2696\uFE0F BE: '+be+'\n\nWin Rate: '+bar+' '+wr+'%\n';if(avgDur)msg+='Avg trade duration: '+(avgDur>=60?Math.floor(avgDur/60)+'h '+(avgDur%60)+'m':avgDur+'m')+'\n';msg+='\n'+(wr>=60?'\uD83D\uDD25 Excellent week.':wr>=45?'\uD83D\uDFE1 Solid week. Stay disciplined.':'\u26A0\uFE0F Tough week. Trust the process.')+'\n\n\u2014 The Slayers Model by Rexroz';await tgSend(msg);const __weekTrades=[...tradeHistory];tradeHistory=[];
  ...
  if(weak.length){
    log('Pair performance flag: '+weak.map(([id,p])=>id+' '+Math.round((p.wins/(p.wins+p.losses))*100)+'% ('+p.wins+'W/'+p.losses+'L)').join(', '));
  }
  saveState();
}
...
async function checkQMRTrades(instId,price,cHigh,cLow){
  ...
  for(let i=activeQMRTrades.length-1;i>=0;i--){
    ...
    if(t.tp1Fired&&!t.slFired&&(isB?hi>=t.tp2:lo<=t.tp2)){
      t.slFired=true;
      await tgQMRUpdate(t,'tp2');try{const[pt,pb]=pushTextFor('tp2',t);sendPushToTrackers(t.sigId,pt,pb);}catch(e){}
    ...
    if(!t.tp1Fired&&(isB?hi>=t.tp1:lo<=t.tp1)){
      ...
      await tgQMRUpdate(t,'tp1');try{const[pt,pb]=pushTextFor('tp1',t);sendPushToTrackers(t.sigId,pt,pb);}catch(e){}
      ...
      if(t.beFired&&(isB?lo<=t.sl:hi>=t.sl)){
        t.slFired=true;
        await tgQMRUpdate(t,'be_close');try{const[pt,pb]=pushTextFor('be_close',t);sendPushToTrackers(t.sigId,pt,pb);}catch(e){}try{const[pt,pb]=pushTextFor('be_close',t);sendPushToTrackers(t.sigId,pt,pb);}catch(e){}
        const winR=computeR(t,t.tp2);
        tradeHistory.push({instId:t.instId,type:t.type,tf:t.tf,outcome:'WIN',rMultiple:winR,time:new Date().toISOString(),duration});
        if(t.isElite&&winR>=2.5){t.rMultiple=winR;await tgTradeOfWeek(t);}
        dailyOutcomeLog.push({id:t.instId,name:t.instName,tf:t.tf,type:t.type,outcome:'WIN',time:new Date().toISOString()});
    ...
    if(!t.beFired&&(isB?price>=t.beLevel:price<=t.beLevel)){
      ...
      await tgQMRUpdate(t,'be');try{const[pt,pb]=pushTextFor('be',t);sendPushToTrackers(t.sigId,pt,pb);}catch(e){}
    }
    ...
    if(t.tp1Fired&&!t.trailSuggested){
      ...
      if((isB?price>=trailTrigger:price<=trailTrigger)){
        ...
        await tgQMRUpdate(t,'trail');try{const[pt,pb]=pushTextFor('trail',t);sendPushToTrackers(t.sigId,pt,pb);}catch(e){}
      }
    }
    ...
    if(!t.slFired&&(isB?lo<=t.sl:hi>=t.sl)){
      ...
      if(t.tp1Fired){
        ...
        await tgQMRUpdate(t,'be_sl');try{const[pt,pb]=pushTextFor('be_sl',t);sendPushToTrackers(t.sigId,pt,pb);}catch(e){}
        ...
        await tgQMRUpdate(t,'sl');try{const[pt,pb]=pushTextFor('sl',t);sendPushToTrackers(t.sigId,pt,pb);}catch(e){}
...
async function runScan(manual=false){
  ...
  if(!isWeekend()&&h===22&&m<30&&lastEOD!==today){
    lastEOD=today;
    // Remove closed/completed signals from the app feed — keeps only active open trades
    const before=appSignalFeed.length;
    appSignalFeed=appSignalFeed.filter(s=>!s.outcome);
    if(before!==appSignalFeed.length)log(`EOD feed cleanup: removed ${before-appSignalFeed.length} closed signals, ${appSignalFeed.length} active remain`);
    saveState();
      ...
      for(const tf of QMR_TFS){
        try{
          ...
          for(const qmr of qmrs){
            ...
            const rr1_q=slD_q>0?Math.abs(tp1_q-qmr.qmLevel)/slD_q:0;
            if(rr1_q<1.2){log('QMR suppressed (weak TP1 reward '+rr1_q.toFixed(2)+'R): '+inst.id+' '+qmr.type);continue;}
            ...
            if(!qmrSeen.has(key)){
              ...
              const STRICT_PAIRS=['GBPNZD','GBPAUD','EURGBP'];
              ...
              if(qmrTF==='4H'){
                ...
                refinedEntry=refine1HEntry(c,qmr.type,qmr.qmLevel,sl_q);
              ...
              activeQMRTrades.push({sigId:sigId,instId:inst.id,instName:instObj.name,tf:qmrTF,type:qmr.type,qmLevel:entry2,sl:sl2,tp1:tp1_q,tp2:tp2_2,beLevel:isB2?entry2+slDist2*1.3:entry2-slDist2*1.3,origSL:sl2,isElite:qmr.criteria.score>=4,eliteFactors:qmr.criteria.factors,dailyPOI:qmr.dailyPOI||null,dec:instObj.dec,beFired:false,tp1Fired:false,tp2Fired:false,slFired:false,trailSuggested:false,openTime:Date.now()});
...
async function sendPushToTrackers(signalId,title,body,url){
