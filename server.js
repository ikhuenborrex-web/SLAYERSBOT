...
if(!API_KEY||!TG_TOKEN||!TG_CHAT)console.warn('⚠️  Missing env vars.');
...
// ── UPDATE 4: Pair-specific session mapping ───────────────────────
// Each pair only alerts during its naturally active hours (UTC)
// No pair is ever fully blocked — just mapped to its liquid session
const PAIR_SESSIONS = {
  EURUSD:{s:7, e:22},  // London + NY
  GBPUSD:{s:7, e:22},  // London + NY
  USDJPY:{s:0, e:22},  // Asian + London + NY
  AUDUSD:{s:0, e:16},  // Asian + London (most active)
  EURJPY:{s:0, e:22},  // Asian + London + NY
  GBPJPY:{s:0, e:22},  // Asian + London + NY
  XAUUSD:{s:7, e:22},  // London + NY
  US30:  {s:13,e:22},  // NY only (market hours)
  SP500: {s:13,e:22},  // NY only
  BTCUSD:{s:0, e:24},  // 24/7 — no filter
};

// ── UPDATE 1: Currency map for news filter ───────────────────────
...
let recentCRTAlerts = [];
let alertLog        = [];
// UPDATE 3: Win rate tracker
let tradeHistory    = [];  // {instId,type,tf,entry,outcome,time}
let dailyAlertLog   = [];  // reset each day
// UPDATE 1: News cache
let newsCache       = [];
let lastNewsFetch   = 0;
...
async function fetchNewsEvents(){
  try{
    ...
    log(`News cache updated: ${newsCache.length} high-impact events this week`);
  }catch(e){log('News fetch error: '+e.message);}
}
function isNewsBlocked(instId){
  const currencies=PAIR_CURRENCIES[instId]||[];
  const now=Date.now(),win=30*60*1000; // 30 min window
  return newsCache.some(ev=>{
    if(!currencies.includes(ev.country))return false;
    try{const t=new Date(ev.date).getTime();return Math.abs(now-t)<win;}
    catch{return false;}
  });
}
function getUpcomingNews(instId){
  const currencies=PAIR_CURRENCIES[instId]||[];
  const now=Date.now(),win=30*60*1000;
  return newsCache.filter(ev=>{
    if(!currencies.includes(ev.country))return false;
    try{const t=new Date(ev.date).getTime();return t>now&&t-now<win;}
    catch{return false;}
  });
}
...
function checkPremiumDiscount(c,type,qmLevel){
  ...
  const mid=(hi+lo)/2;
  return type==='BULLISH'?qmLevel<mid:qmLevel>mid;
}
...
function headSweptLiquidity(c,head,type){
  ...
  if(type==='BEARISH'){
    for(let i=0;i<refC.length-2;i++){
      for(let j=i+2;j<refC.length;j++){
        if(Math.abs(refC[j].high-refC[i].high)/refC[i].high<tol&&head.p>=refC[i].high){eqSwept=true;break;}
      }if(eqSwept)break;
    }
    if(eqSwept)return true;
    return refC.some(x=>head.p>x.high*1.0003);
  } else {
    for(let i=0;i<refC.length-2;i++){
      for(let j=i+2;j<refC.length;j++){
        if(Math.abs(refC[j].low-refC[i].low)/refC[i].low<tol&&head.p<=refC[i].low){eqSwept=true;break;}
      }if(eqSwept)break;
    }
    if(eqSwept)return true;
    return refC.some(x=>head.p<x.low*0.9997);
  }
}
function validateQMRCriteria(c,type,head,qmSwing,atr,sH,sL){
  ...
  const disp=first3.some(x=>{
    const body=Math.abs(x.close-x.open),range=x.high-x.low;
    const dirOk=bearD?x.close<x.open:x.close>x.open;
    return dirOk&&(body>atr*0.85||(range>0&&body/range>0.65));
  });
  if(disp){sc++;F.push('Displacement');}
  const cp=c[c.length-1].close;
  const mss=type==='BEARISH'?cp<=qmSwing.p*1.004:cp>=qmSwing.p*0.996;
  if(mss){sc++;F.push('MSS');}
  const engRange=atr*3;
  const engRef=type==='BEARISH'?sL:sH;
  if(engRef.filter(s=>Math.abs(s.p-qmSwing.p)<engRange&&s.i<head.i).length>=2){sc++;F.push('Eng. Liq');}
...
function detectQMR(c){
  ...
  for(let h=sH.length-1;h>=1;h--){
    ...
    const postW=c.slice(head.i+1,Math.min(c.length,head.i+13));
    if(!postW.some(x=>x.close<hl.p))continue;
    ...
    const obNear=obs.bear.find(z=>Math.abs((z.top+z.bottom)/2-hl.p)<atr*2.0);
    const fvgNear=fvgs.bear.find(z=>Math.abs((z.top+z.bottom)/2-hl.p)<atr*2.0);
    if(!obNear&&!fvgNear)continue;
    if(obNear)crit.factors.push('Reclaimed OB');else if(fvgNear)crit.factors.push('FVG at QM');
    let trueHigh=head.p;
    for(let k=head.i;k<c.length;k++){if(c[k].high>trueHigh)trueHigh=c[k].high;}
    const retestSL=trueHigh+atr*0.25;
    results.push({type:'BEARISH',qmLevel:hl.p,head:head.p,cp,atr,criteria:crit,atLevel:true,drawOnLiquidity:null,retestSL});
    break;
  }
  ...
  for(let l=sL.length-1;l>=1;l--){
    ...
    const postW=c.slice(head.i+1,Math.min(c.length,head.i+13));
    if(!postW.some(x=>x.close>lh.p))continue;
    ...
    const obs=detectOB(c),fvgs=detectFVG(c);
    const obNear=obs.bull.find(z=>Math.abs((z.top+z.bottom)/2-lh.p)<atr*2.0);
    const fvgNear=fvgs.bull.find(z=>Math.abs((z.top+z.bottom)/2-lh.p)<atr*2.0);
    if(!obNear&&!fvgNear)continue;
    if(obNear)crit.factors.push('Reclaimed OB');else if(fvgNear)crit.factors.push('FVG at QM');
    let trueLow=head.p;
    for(let k=head.i;k<c.length;k++){if(c[k].low<trueLow)trueLow=c[k].low;}
    const retestSL=trueLow-atr*0.25;
    results.push({type:'BULLISH',qmLevel:lh.p,head:head.p,cp,atr,criteria:crit,atLevel:true,drawOnLiquidity:null,retestSL});
    break;
  }
  return results;
}
...
async function tgSend(text){
  if(!TG_TOKEN||!TG_CHAT){log('Telegram not configured');return;}
  try{
...
async function tgCRT(id,crt,sc,wBias,sess,newsWarn){
  ...
  const bArrow=wBias==='BULLISH'?'\u2191':wBias==='BEARISH'?'\u2193':'-';
  let msg=
    '\u26A1 SLAYERS CRT ALERT \u26A1\n'+(strMap[sc.strength]||'\uD83D\uDFE1 VALID')+'\n'+'='.repeat(28)+'\n'+
    '\uD83D\uDCCA '+id+' \u00B7 DAILY\n'+(bear?'\uD83D\uDD34 SELL SETUP':'\uD83D\uDFE2 BUY SETUP')+'\n'+(bear?'\uD83D\uDCC9 BEARISH CRT':'\uD83D\uDCC8 BULLISH CRT')+' \u2014 '+(bear?'Supply':'Demand')+' Sweep\n\n'+
    ...
    '\uD83D\uDCC5 Weekly Bias: '+wBias+' '+bArrow+'\n'+(sess==='CLOSED'?'\uD83D\uDD34':'\uD83D\uDFE2')+' Session: '+sess+'\n\n';
...
async function tgQMR(id,tf,qmr,htfBias,newsWarn,sessWarn){
  ...
  const baseTP2=bear?entry-slDist*4.5:entry+slDist*4.5;
  const tp2=bear?Math.min(baseTP2,tp1-slDist):Math.max(baseTP2,tp1+slDist);
  const rr1=slDist>0?(Math.abs(tp1-entry)/slDist).toFixed(1):'--';
  const rr2=slDist>0?(Math.abs(tp2-entry)/slDist).toFixed(1):'--';
  const dolLabel=dol?dol.label:'Draw on Liquidity';
  const slLabel=bear?'above protected high':'below protected low';

  // UPDATE 2: HTF confluence label
  let htfLine='';
  if(htfBias&&htfBias!=='NEUTRAL'){
    const agrees=(bear&&htfBias==='BEARISH')||((!bear)&&htfBias==='BULLISH');
    htfLine='\n'+(agrees?'\uD83D\uDD25 HTF Aligned: Weekly '+htfBias+' \u2014 HIGH PROBABILITY':'\u26A0\uFE0F Counter-trend: Weekly '+htfBias+' \u2014 Trade carefully');
  }
  ...
  let msg=
    ...
    '\uD83C\uDFAF Extended: '+tp2.toFixed(p)+' (1:'+rr2+'R)\n\n'+
...
async function tgDailyBriefing(scanData){
  ...
  for(const inst of CRT_INSTS){
    ...
    const bA=bias==='BULLISH'?'\u2191':bias==='BEARISH'?'\u2193':'\u2192';
    msg+='\uD83D\uDCCA '+inst.name+' \u2014 '+d.price+'\n'+bI+' '+bias+' '+bA+'\n\n';
  }
  msg+='\uD83D\uDCC5 SESSIONS (UTC)\n\uD83D\uDFE2 London: 07:00\u201316:00\n\uD83D\uDFE2 New York: 13:00\u201322:00\n\uD83D\uDD25 Overlap: 13:00\u201316:00\n\n\u26A0\uFE0F Wait for CRT or QMR confirmation.\n\u2014 The Slayers Model by Rexroz';
  await tgSend(msg);
  dailyAlertLog=[]; // reset daily log
}

// ── UPDATE 5: End-of-Day Summary ─────────────────────────────────
async function tgEODSummary(){
  const now=new Date(),days=['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
  const dateStr=days[now.getUTCDay()]+' '+now.getUTCDate();
  const crtToday=dailyAlertLog.filter(a=>a.type==='CRT');
  const qmrToday=dailyAlertLog.filter(a=>a.type==='QMR');
  if(crtToday.length===0&&qmrToday.length===0){return;} // nothing to report
  let msg='\uD83D\uDCCA SLAYERS END OF DAY\n'+dateStr+' | 22:00 UTC\n'+'='.repeat(28)+'\n\n';
  msg+='Signals fired today: '+(crtToday.length+qmrToday.length)+'\n';
  if(crtToday.length>0){
    msg+='\n\uD83D\uDCB9 CRT ('+crtToday.length+'):\n';
    crtToday.forEach(a=>msg+=(a.dir==='BULLISH'?'\uD83D\uDFE2':'\uD83D\uDD34')+' '+a.id+' \u2014 Score: '+a.score+'/10\n');
  }
  if(qmrToday.length>0){
    msg+='\n\uD83D\uDD04 QMR ('+qmrToday.length+'):\n';
    qmrToday.forEach(a=>msg+=(a.dir==='BULLISH'?'\uD83D\uDFE2':'\uD83D\uDD34')+' '+a.id+' '+a.tf+' \u2014 '+a.score+'/4\n');
  }
  msg+='\n\uD83D\uDD14 Review your charts. Manage open positions.\n\u2014 The Slayers Model by Rexroz';
  await tgSend(msg);
}

// ── UPDATE 3: Weekly Performance Summary ─────────────────────────
async function tgWeeklySummary(){
  const total=tradeHistory.length;
  if(total===0){await tgSend('\uD83D\uDCCA WEEKLY SUMMARY\nNo completed trades this week.\n\u2014 The Slayers Model by Rexroz');return;}
  const tp1=tradeHistory.filter(t=>t.outcome==='TP1'||t.outcome==='TP2').length;
  const tp2=tradeHistory.filter(t=>t.outcome==='TP2').length;
  const sl=tradeHistory.filter(t=>t.outcome==='SL').length;
  const winRate=Math.round((tp1/total)*100);
  ...
  let msg='\uD83D\uDCCA SLAYERS WEEKLY PERFORMANCE\nWeek ending '+days[now.getUTCDay()]+' '+now.getUTCDate()+' '+months[now.getUTCMonth()]+'\n'+'='.repeat(28)+'\n\n';
  msg+='\uD83D\uDD25 Total Signals: '+total+'\n';
  msg+='\u2705 TP1 Reached: '+tp1+' ('+winRate+'%)\n';
  msg+='\uD83D\uDCB0 Full Target: '+tp2+'\n';
  msg+='\uD83D\uDEAB Stop Loss: '+sl+'\n\n';
  msg+='Win Rate: '+bar+' '+winRate+'%\n\n';
  if(winRate>=60)msg+='\uD83D\uDD25 Excellent week. System performing strongly.\n';
  else if(winRate>=45)msg+='\uD83D\uDFE1 Solid week. Stay disciplined.\n';
  else msg+='\u26A0\uFE0F Tough week. Review setups. Trust the process.\n';
  msg+='\n\u2014 The Slayers Model by Rexroz';
  await tgSend(msg);
  tradeHistory=[]; // reset after weekly report
}
...
async function checkQMRTrades(instId,price,cHigh,cLow){
  ...
  for(let i=activeQMRTrades.length-1;i>=0;i--){
    ...
    if(!t.slFired&&(isB?lo<=t.sl:hi>=t.sl)){
      ...
      // UPDATE 3: Record outcome
      tradeHistory.push({instId:t.instId,type:t.type,tf:t.tf,entry:t.qmLevel,outcome:'SL',time:new Date().toISOString()});
    ...
    if(!t.tp1Fired&&(isB?hi>=t.tp1:lo<=t.tp1)){
      ...
      tradeHistory.push({instId:t.instId,type:t.type,tf:t.tf,entry:t.qmLevel,outcome:'TP1',time:new Date().toISOString()});
    }
    if(!t.tp2Fired&&(isB?hi>=t.tp2:lo<=t.tp2)){
      t.tp2Fired=true;await tgQMRUpdate(t,'tp2');
      // Update TP1 record to TP2 if TP2 hit
      const tp1Rec=tradeHistory.findLast(r=>r.instId===t.instId&&r.outcome==='TP1');
      if(tp1Rec)tp1Rec.outcome='TP2';
      else tradeHistory.push({instId:t.instId,type:t.type,tf:t.tf,entry:t.qmLevel,outcome:'TP2',time:new Date().toISOString()});
...
async function runScan(manual=false){
  if(!API_KEY){log('No API key configured');return;}
  ...
  const today=now.toUTCString().slice(0,16);
  const dow=now.getUTCDay();

  // Daily briefing 07:00 UTC weekdays
  if(!isWeekend()&&h===7&&m<30&&lastBriefing!==today){
    lastBriefing=today;await tgDailyBriefing(briefingData);
  }
  // UPDATE 5: EOD summary 22:00 UTC weekdays
  if(!isWeekend()&&h===22&&m<30&&lastEOD!==today){
    lastEOD=today;await tgEODSummary();
  }
  // UPDATE 3: Weekly summary Sunday 20:00 UTC
  const thisWeek=now.toISOString().slice(0,10);
  if(dow===0&&h===20&&m<30&&lastWeeklySummary!==thisWeek){
    lastWeeklySummary=thisWeek;await tgWeeklySummary();
  }
  ...
  for(const inst of CRT_INSTS){
    ...
    try{
      ...
      if(crt&&!(isWeekend()&&inst.id!=='BTCUSD')){
        const liq=detectLiquidity(c,crt.sweep,crt.type);
        const sc=calcCRTScore(crt,sdZ,obs,fvgs,brk,wc.bias,sess,prevDay,prevWeek,fib,struct,liq);
        if(sc.score>=CRT_MIN){
          const key=inst.id+'-DAILY-CRT-'+crt.dt;
          if(!crtSeen.has(key)){
            crtSeen.add(key);
            // UPDATE 1: news warning (don't block CRT — it's daily, just warn)
            const newsWarn=isNewsBlocked(inst.id);
            log(`CRT ALERT: ${inst.id} DAILY ${crt.type} Score:${sc.score}/10${newsWarn?' [NEWS WARNING]':''}`);
      ...
      for(const tf of QMR_TFS){
        try{
          ...
          for(const qmr of qmrs){
            ...
            if(!qmrSeen.has(key)){
              qmrSeen.add(key);
              // UPDATE 1: News filter — block QMR if high impact news within 30min
              if(isNewsBlocked(inst.id)){
                log(`QMR BLOCKED (news): ${inst.id} ${tf.toUpperCase()} ${qmr.type}`);
                continue; // skip this alert — too close to news
              }
              // UPDATE 4: Session check — fire but add warning if outside prime session
              const inSess=isPairInSession(inst.id);
              const sessWarn=!inSess;
              // UPDATE 2: HTF bias for QMR
              const htfBias=weeklyCache[inst.id]?.bias||'NEUTRAL';
              log(`QMR ALERT: ${inst.id} ${tf.toUpperCase()} ${qmr.type} Level:${qmr.qmLevel} HTF:${htfBias}${sessWarn?' [OFF-SESSION]':''}`);
              await tgQMR(inst.id,tf==='1h'?'1H':'4H',qmr,htfBias,false,sessWarn);
              ...
              const entry2=qmr.qmLevel;
              const sl2=qmr.retestSL!=null?qmr.retestSL:(isB2?qmr.head-qmr.atr*0.1:qmr.head+qmr.atr*0.1);
              const slDist2=Math.abs(entry2-sl2);
              const dol2=qmr.drawOnLiquidity;
              const tp1_2=dol2?dol2.price:(isB2?entry2+slDist2*3:entry2-slDist2*3);
              const baseTP2_2=isB2?entry2+slDist2*4.5:entry2-slDist2*4.5;
              const tp2_2=isB2?Math.max(baseTP2_2,tp1_2+slDist2):Math.min(baseTP2_2,tp1_2-slDist2);
...
app.get('/',(req,res)=>{
  ...
  const winRate=tradeHistory.length?Math.round((tradeHistory.filter(t=>t.outcome==='TP1'||t.outcome==='TP2').length/tradeHistory.length)*100):0;
  const recentAlerts=alertLog.slice(0,10).map(a=>`<tr><td>${a.time.slice(11,19)}</td><td>${a.type}</td><td>${a.id}</td><td>${a.tf}</td><td style="color:${a.dir==='BULLISH'?'#4ade80':'#f87171'}">${a.dir}</td><td>${a.score}</td></tr>`).join('');
  const activeTrades=activeQMRTrades.map(t=>`<tr><td>${t.instName}</td><td>${t.tf}</td><td style="color:${t.type==='BULLISH'?'#4ade80':'#f87171'}">${t.type}</td><td>${t.qmLevel.toFixed(t.dec)}</td><td>${t.sl.toFixed(t.dec)}</td><td>${t.beFired?'BE&#10003; ':''}${t.tp1Fired?'TP1&#10003; ':''}${t.tp2Fired?'TP2&#10003;':''}</td></tr>`).join('');
  const newsRows=newsCache.slice(0,8).map(e=>`<tr><td>${e.country}</td><td>${e.title}</td><td style="color:#f87171">${e.impact}</td><td>${new Date(e.date).toUTCString().slice(17,22)} UTC</td></tr>`).join('');
  res.send(`<!DOCTYPE html><html><head><title>Slayers v6</title><meta charset="UTF-8"><style>body{background:#080c10;color:#c8d8e8;font-family:monospace;padding:20px}h1{color:#26A69A;letter-spacing:1px}h2{color:#c084fc;margin-top:24px;font-size:14px;letter-spacing:1px}table{border-collapse:collapse;width:100%;margin-top:6px}td,th{border:1px solid #1e2530;padding:6px 10px;font-size:12px}th{color:#26A69A;background:#0c1218}.dim{color:#4a6a7a}.live{color:#4ade80}.gold{color:#F5A623}</style></head><body>
<h1>SLAYERS ALERT SYSTEM v6</h1>
...
<p class="gold">Win Rate (this week): ${winRate}% &nbsp;|&nbsp; Completed trades: ${tradeHistory.length} &nbsp;|&nbsp; News events loaded: ${newsCache.length}</p>
<h2>RECENT ALERTS</h2>
<table><tr><th>Time</th><th>System</th><th>Pair</th><th>TF</th><th>Direction</th><th>Score</th></tr>${recentAlerts||'<tr><td colspan="6" class="dim">No alerts yet</td></tr>'}</table>
<h2>ACTIVE QMR TRADES</h2>
<table><tr><th>Pair</th><th>TF</th><th>Type</th><th>Entry</th><th>SL</th><th>Progress</th></tr>${activeTrades||'<tr><td colspan="6" class="dim">No active trades</td></tr>'}</table>
<h2>HIGH IMPACT NEWS THIS WEEK</h2>
<table><tr><th>Currency</th><th>Event</th><th>Impact</th><th>Time</th></tr>${newsRows||'<tr><td colspan="4" class="dim">No news loaded yet</td></tr>'}</table>
<p class="dim" style="margin-top:24px">The Slayers Model by Rex Roz &nbsp;|&nbsp; v6</p>
</body></html>`);
});

// ═══ START ════════════════════════════════════════════════════════
app.listen(PORT,()=>log(`Status page running on port ${PORT}`));
log('Slayers Alert System v6 starting...');
runScan(true).then(()=>{
  setInterval(()=>runScan(false),CHECK_MS);
  log(`Scan interval: every ${CHECK_MS/60000} minutes`);
});
