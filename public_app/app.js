...
'.tour-tooltip{position:absolute;left:50%;transform:translateX(-50%);width:300px;background:rgba(20,20,22,0.95);backdrop-filter:blur(40px);-webkit-backdrop-filter:blur(40px);border:0.5px solid rgba(255,255,255,0.08);border-radius:18px;padding:22px 20px;animation:slideUp 0.45s cubic-bezier(0.16,1,0.3,1) forwards;box-shadow:0 20px 60px rgba(0,0,0,0.5);z-index:45}'+
...
'.tour-header svg{width:22px;height:22px;fill:none;stroke:#A3E635;stroke-width:1.5;stroke-linecap:round;stroke-linejoin:round}'+
...
function statsOverview(){
  ...
  return '<div style="display:flex;gap:8px;margin-bottom:14px">'+
    ...
    '<div style="display:flex;align-items:baseline;justify-content:center;gap:1px;margin-top:2px">'+
    '<span style="font-size:13px;font-weight:700;color:'+C.lime+'">'+(trTotal>=0?'+':'')+'</span>'+
...
function journalScreen(){
  ...
  return '<div style="display:flex;align-items:center;justify-content:space-between;padding-top:12px;margin-bottom:14px">'+
    ...
    '<div style="text-align:center"><div style="display:flex;align-items:baseline;justify-content:center;gap:1px"><span style="font-size:12px;font-weight:700;color:'+C.lime+'">'+(totalR>=0?'+':'')+'</span><div class="count-up" style="font-size:22px;font-weight:800;color:'+C.lime+';letter-spacing:-0.5px" data-target="'+Math.abs(totalR||0)+'" data-dur="800">0</div><span style="font-size:12px;font-weight:700;color:'+C.lime+'">R</span></div><div style="font-size:9px;color:'+C.text2+';font-weight:500;margin-top:2px">Total R</div></div>'+
...
function onboardingHTML(){
  ...
  return '<div class="tour-overlay show" id="tourOverlay" onclick="event.stopPropagation()"></div>'+
    ...
    (isLast
      ?'<button class="btn-primary" style="padding:8px 18px;font-size:12px;width:auto" onclick="endTour(true)">Got it</button>'
...
window.nextTourStep=function(){
  if(state.onboardingStep<tourSteps.length-1){state.onboardingStep++;render();requestAnimationFrame(positionTourHighlight);}
};
window.prevTourStep=function(){
  if(state.onboardingStep>0){state.onboardingStep--;render();requestAnimationFrame(positionTourHighlight);}
};
