  ...
  return `
    ...
    <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:12px">
      ${['all', 'wins', 'losses', 'BE', ...tags].map(t => {
        ...
        return `<span onclick="setJournalFilter('${t}')" style="font-size:10px;font-weight:600;padding:5px 12px;border-radius:99px;cursor:pointer;background:${active ? MINT_DIM : 'rgba(255,255,255,0.03)'};color:${active ? MINT : GREY};border:0.5px solid ${active ? MINT + '33' : 'rgba(255,255,255,0.05)'}">${t === 'all' ? 'All' : t.charAt(0).toUpperCase() + t.slice(1)}</span>`;
      }).join('')}
    </div>
      ...
      <div class="card" style="padding:12px 16px;animation-delay:${i*0.03}s">
        <div style="display:flex;gap:12px;align-items:flex-start">
          ...
          <div style="flex:1;min-width:0">
            ...
            ${e.tags && e.tags.length ? `<div style="display:flex;gap:4px;margin-top:5px">${e.tags.map(t => `<span style="font-size:8.5px;padding:2px 8px;border-radius:99px;background:${MINT_DIM};color:${MINT};border:0.5px solid ${MINT}1A">${t}</span>`).join('')}</div>` : ''}
          </div>
        </div>
      </div>
...
function journalEntryForm() {
  return `
    <div class="card" style="border-color:${MINT}33;animation-delay:0s">
  ...
  return `
    ...
    <div style="display:flex;gap:6px;overflow-x:auto;padding-bottom:4px;margin-bottom:14px;scrollbar-width:none">
      ${['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((d, i) => {
        ...
        return `<div style="text-align:center;padding:6px 14px;border-radius:99px;cursor:pointer;background:${active ? MINT_DIM : 'transparent'};border:0.5px solid ${active ? MINT + '33' : 'rgba(255,255,255,0.04)'}">
...
function render() {
  ...
  app.innerHTML = `
    <div style="display:flex;flex-direction:column;height:100dvh;background:${BLACK};color:${WHITE}">
      <div style="flex-shrink:0;height:52px;display:flex;align-items:flex-end;justify-content:space-between;padding:0 20px 8px;font-size:11px;font-weight:600;letter-spacing:0.5px">
        <span>9:41</span>
        <div style="display:flex;gap:6px;align-items:center">
          <svg width="14" height="10" viewBox="0 0 14 10"><rect x="0.5" y="0.5" width="13" height="9" rx="1.5" fill="none" stroke="currentColor" stroke-width="0.8"/><rect x="2" y="2" width="10" height="6" rx="0.5" fill="currentColor" opacity="0.3"/></svg>
          <svg width="14" height="10" viewBox="0 0 14 10"><path d="M1 5L5 9L13 1" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/></svg>
        </div>
      </div>
      ...
      <div id="tabbar" style="flex-shrink:0;display:flex;position:relative;border-top:0.5px solid rgba(255,255,255,0.04);background:rgba(10,14,18,0.88);backdrop-filter:blur(32px) saturate(1.4);-webkit-backdrop-filter:blur(32px) saturate(1.4);padding:6px 0 calc(6px + env(safe-area-inset-bottom,0px));z-index:10">
        ...
        ${['overview','journal','calendar','settings'].map(t => `
          <button class="tab-btn${state.tab===t?' active':''}" data-tab="${t}" style="color:${state.tab===t?MINT:MUTED}">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">${ICONS[t==='overview'?'grid':t==='journal'?'book':t==='calendar'?'cal':'gear']}</svg>
...
function overviewScreen() {
  ...
  return `
    ...
    <div class="section-h">Market Pulse</div>
    ${marketPulseRow()}
    ${statsOverview()}
...
function activeTradeWidget(t) {
  ...
  return `
    <div class="card-glass" style="animation-delay:0s">
      ...
      <div style="display:flex;justify-content:space-between;margin-top:10px;align-items:center">
        <div style="display:flex;gap:6px">
          ${t.tp1Fired ? `<span style="font-size:8px;font-weight:600;padding:2px 8px;border-radius:99px;background:${MINT_DIM};color:${MINT}">TP1 ✓</span>` : ''}
          ${t.beFired ? `<span style="font-size:8px;font-weight:600;padding:2px 8px;border-radius:99px;background:${RED_DIM};color:${RED}">BE ✓</span>` : ''}
          ${t.trailActive ? `<span style="font-size:8px;font-weight:600;padding:2px 8px;border-radius:99px;background:${GOLD_DIM};color:${GOLD}">Trailing</span>` : ''}
        </div>
