...
'.card{background:rgba(15,21,26,0.72);border:0.5px solid rgba(255,255,255,0.06);border-radius:24px;padding:16px;margin-bottom:12px;box-shadow:0 4px 20px rgba(0,0,0,0.25);animation:springUp 0.6s cubic-bezier(0.16,1,0.3,1) both;transition:transform 0.2s}'+
'.card:active{transform:scale(0.985)}'+
'.card-glass{background:rgba(15,21,26,0.72);backdrop-filter:blur(16px) saturate(1.3);-webkit-backdrop-filter:blur(16px) saturate(1.3);border:0.5px solid rgba(255,255,255,0.06);border-radius:24px;padding:18px;margin-bottom:14px;box-shadow:0 4px 20px rgba(0,0,0,0.25)}'+
'.mono{font-family:SF Mono,JetBrains Mono,monospace;font-weight:500;letter-spacing:-0.3px}'+
'.pill{display:inline-flex;align-items:center;gap:4px;font-size:9px;font-weight:600;letter-spacing:0.4px;padding:3px 12px;border-radius:99px;text-transform:uppercase}'+
...
'.toggle::after{content:"";position:absolute;top:2px;left:2px;width:20px;height:20px;border-radius:99px;background:white;transition:transform 0.3s cubic-bezier(0.16,1,0.3,1)}'+
...
'.nav-indicator{position:absolute;top:-0.5px;height:2px;border-radius:99px;background:#2DD4BF;transition:left 0.35s cubic-bezier(0.16,1,0.3,1),width 0.35s cubic-bezier(0.16,1,0.3,1);box-shadow:0 0 20px rgba(45,212,191,0.2)}'+
...
'.ptr-text{font-size:11px;color:#7C8A87;font-weight:500;letter-spacing:0.1px}'+
...
function signalCard(s){
  ...
  var tierDim=s.tier==='ELITE'?C.emeraldSoft:C.goldSoft;
  var criteria=(s.criteria||[]).map(function(c){
    return '<span style="font-size:10px;color:'+C.emerald+';background:'+C.emeraldSoft+';border:0.5px solid '+C.emerald+'44;border-radius:99px;padding:3px 10px;font-weight:600;display:inline-block;margin:0 6px 6px 0">'+c+'</span>';
  }).join('');
  return '<div class="card" onclick="openDetail(\''+s.id+'\')" style="cursor:pointer;border-left:2.5px solid '+tierColor+';animation-delay:'+(Math.random()*0.2)+'s">'+
    ...
    pill(s.tier,tierColor,tierDim,s.tier==='ELITE'?'pulse-ring':'')+
    pill(isBuy?'BUY':'SELL',isBuy?C.emerald:C.rose,isBuy?C.emeraldSoft:C.roseSoft)+
    '<span style="margin-left:auto;font-size:10px;color:'+C.text2+'">'+timeAgo(s.time)+'</span>'+
    '</div>'+
    '<div style="display:flex;gap:12px;font-size:12px;color:'+C.text2+'">'+
    '<span><span style="color:'+C.text2+'">Entry</span> <span style="font-weight:700;color:'+C.white+'">'+fmt(s.entry)+'</span></span>'+
    '<span><span style="color:'+C.text2+'">SL</span> <span style="font-weight:700;color:'+C.rose+'">'+fmt(s.sl)+'</span></span>'+
    (s.tp1?'<span><span style="color:'+C.text2+'">TP1</span> <span style="font-weight:700;color:'+C.gold+'">'+fmt(s.tp1)+'</span></span>':'')+
    '</div>'+
    '<div style="margin-top:8px">'+criteria+'</div>'+
    (s.dailyPOI?'<div style="font-size:11px;color:'+C.emerald+';font-weight:600;margin-top:6px">\uD83C\uDFDB '+s.dailyPOI+'</div>':'')+
    '<div style="color:'+C.text2+';font-size:10px;margin-top:10px;text-align:center">Tap for full chart \u2192</div>'+
    '</div>';
}
...
function detailPage(s){
  ...
  var criteriaList=(s.criteria||[]).map(function(c){
    return '<div style="display:flex;align-items:center;gap:8px;padding:6px 0">'+icon('check',C.emerald,15)+'<span style="font-size:13px;color:'+C.white+'">'+c+'</span></div>';
  }).join('');
  ...
  return '<div style="display:flex;flex-direction:column;height:100dvh;background:'+C.bg+';color:'+C.white+'">'+
    ...
    '<button onclick="closeDetail()" style="background:rgba(15,21,26,0.72);border:0.5px solid '+C.border+';border-radius:99px;width:34px;height:34px;display:flex;align-items:center;justify-content:center;cursor:pointer">'+icon('back',C.white,16)+'</button>'+
    ...
    pill(s.tier,tierColor,s.tier==='ELITE'?C.emeraldSoft:C.goldSoft)+
    pill(s.type,isBuy?C.emerald:C.rose,isBuy?C.emeraldSoft:C.roseSoft)+
    '</div>'+chartHtml+
    ...
    (s.refinedEntry?'<div style="display:flex;justify-content:space-between;padding:9px 0"><span style="color:'+C.gold+'">'+icon('target',C.gold,13)+' Refined Entry</span>'+mono(fmt(s.refinedEntry),C.gold,16)+'</div>':'')+
    '<div style="display:flex;justify-content:space-between;padding:9px 0"><span style="color:'+C.rose+'">'+icon('shield',C.rose,13)+' Stop Loss</span>'+mono(fmt(s.sl),C.rose,16)+'</div>'+
    ...
    (s.counterTrend?'<div style="background:'+C.goldSoft+';border:0.5px solid '+C.gold+'55;border-radius:14px;padding:14px;margin-bottom:18px;display:flex;gap:8px"><span>'+icon('warn',C.gold,16)+'</span><span style="color:'+C.gold+';font-size:12.5px;font-weight:600">Counter-trend setup \u2014 '+(s.htfBias||'')+'. Reduce position size.</span></div>':'')+
    '<button onclick="toggleTrack(\''+s.id+'\','+!!s.isTracked+')" style="width:100%;background:'+(s.isTracked?C.emeraldSoft:'transparent')+';border:0.5px solid '+(s.isTracked?C.emerald:C.border)+';border-radius:99px;padding:14px 0;display:flex;align-items:center;justify-content:center;gap:8px;cursor:pointer;margin-bottom:12px">'+
    icon(s.isTracked?'check':'target',s.isTracked?C.emerald:C.text2,16)+
    '<span style="color:'+(s.isTracked?C.emerald:C.text2)+';font-weight:700;font-size:13px">'+(s.isTracked?'Tracking \u2014 you will get updates on this trade':"I'm in this trade \u2014 notify me")+'</span></button>'+
    '<button onclick="window.open(\'https://slayerbotcalculator.netlify.app/#'+s.pair.replace('/','')+','+s.entry+','+s.sl+'\',\'_blank\')" style="width:100%;background:'+C.emerald+';border:none;border-radius:99px;padding:15px 0;display:flex;align-items:center;justify-content:center;gap:8px;cursor:pointer;margin-bottom:24px">'+
    icon('calc',C.bg,16)+'<span style="color:'+C.bg+';font-weight:800;font-size:14px">Calculate Position Size</span></button>'+
    '</div></div></div>';
}
...
function confluenceScreen(){
  ...
  return state.confluence.map(function(p){
    ...
    var wc=p.weeklyBias==='BULLISH'?C.emerald:p.weeklyBias==='BEARISH'?C.rose:C.text2;
    var dc=p.dailyTrend==='BULLISH'?C.emerald:p.dailyTrend==='BEARISH'?C.rose:C.text2;
    var tc=p.convictionLabel==='ELITE'?C.emerald:p.convictionLabel==='STRONG'?C.gold:p.convictionLabel==='VALID'?C.gold:C.text2;
    var userTag=p.userInTrade
      ?'<span style="font-size:9px;color:'+C.emerald+';background:'+C.emeraldSoft+';border-radius:99px;padding:2px 8px;font-weight:700">YOURS'+(p.activeTradeProgress?.tp1Fired?' TP1\u2713':'')+(p.activeTradeProgress?.beFired?' BE\u2713':'')+'</span>'
      :'';
    return '<div class="card" style="border-left:2.5px solid '+tc+';animation-delay:'+(Math.random()*0.15)+'s">'+
      ...
      (p.biasRelation==='ALIGNED'?'<span style="font-size:10px;border:0.5px solid '+C.emerald+'44;background:'+C.emeraldSoft+';border-radius:99px;padding:3px 10px;color:'+C.emerald+';font-weight:600">ALIGNED</span>'
        :p.biasRelation==='CONFLICT'?'<span style="font-size:10px;border:0.5px solid '+C.rose+'44;background:'+C.roseSoft+';border-radius:99px;padding:3px 10px;color:'+C.rose+';font-weight:600">CONFLICT</span>'
        :'<span style="font-size:10px;border:0.5px solid '+C.gold+'44;background:'+C.goldSoft+';border-radius:99px;padding:3px 10px;color:'+C.gold+';font-weight:600">MIXED</span>')+
      pill(p.convictionLabel,tc,tc+'22',p.convictionLabel==='ELITE'?'pulse-ring':'')+'</div>'+
      (p.signalDir!=='NONE'?'<div style="display:flex;align-items:center;gap:6px;margin-bottom:6px">'+icon(isBull?'up':'down',isBull?C.emerald:C.rose,14)+'<span style="font-weight:600;font-size:12px;color:'+C.white+'">'+(p.signalDir==='BULLISH'?'BUY':'SELL')+' signal</span></div>':'<div style="color:'+C.text2+';font-size:11px;margin-bottom:6px">No active signals</div>')+
      (p.factors.length?'<div style="display:flex;gap:4px;flex-wrap:wrap">'+p.factors.map(function(f){return '<span style="font-size:9px;color:'+C.emerald+';background:'+C.emeraldSoft+';border-radius:99px;padding:2px 8px">\u2713 '+f+'</span>';}).join('')+'</div>':'')+
      '</div>';
  }).join('');
}
...
function statsOverview(){
  ...
  return '<div style="display:flex;gap:8px;margin-bottom:14px">'+
    ...
    '<div class="count-up" style="font-size:22px;font-weight:800;color:'+C.emerald+';margin-top:2px;letter-spacing:-0.5px" data-target="'+(st.winRate||0)+'">0</div></div>'+
    ...
    '<div class="count-up" style="font-size:22px;font-weight:800;color:'+C.gold+';margin-top:2px;letter-spacing:-0.5px" data-target="'+(st.totalR||0)+'" data-dur="800">0</div></div>'+
    ...
    '<div class="count-up" style="font-size:22px;font-weight:800;margin-top:2px;letter-spacing:-0.5px" data-target="'+state.active.length+'">0</div></div></div>';
}
...
function equityChart(entries){
  ...
  var color=lastVal>=0?C.emerald:C.rose;
...
function rBarChart(entries){
  ...
  for(var i=0;i<slice.length;i++){
    ...
    var c=r>0?C.emerald:r<0?C.rose:C.gold;
...
function journalScreen(){
  ...
  function renderCells(outcomes,getDate,getOutcome){
    ...
    for(var i=0;i<28;i++){
      ...
      var color=outcome==='WIN'||outcome==='TP1'||outcome==='TP2'?C.emerald:outcome==='SL'?C.rose:outcome==='BE'?C.gold:'rgba(255,255,255,0.04)';
  ...
  function heatmapCard(label,outcomes,getDate,getOutcome){
    ...
    return '<div class="card" style="animation-delay:0.05s">'+
      ...
      '<span><span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:'+C.emerald+';vertical-align:middle;margin-right:2px"></span>W</span>'+
      '<span><span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:'+C.rose+';vertical-align:middle;margin-right:2px"></span>L</span>'+
      '<span><span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:'+C.gold+';vertical-align:middle;margin-right:2px"></span>BE</span>'+
      '</div></div>'+
  ...
  function tagHtml(tag){
    ...
    var bg,col;
    if(isEarly){bg=C.roseSoft;col=C.rose;}
    else if(isFeeling){bg=C.goldSoft;col=C.gold;}
    else{bg=C.emeraldSoft;col=C.emerald;}
    return '<span style="font-size:8.5px;padding:2px 8px;border-radius:99px;background:'+bg+';color:'+col+'">'+(emojiMap[tag]||'')+' '+tag+'</span>';
  }
  function flagHtml(f){return '<span style="font-size:8.5px;padding:2px 8px;border-radius:99px;background:'+C.goldSoft+';color:'+C.gold+';font-weight:600">\u26A0 '+f+'</span>';}

  function entryHtml(e){
    var outcome=e.outcome;
    var isWin=outcome==='WIN'||outcome==='TP1'||outcome==='TP2';
    var iconColor=isWin?C.emerald:outcome==='SL'?C.rose:C.gold;
    var iconBg=isWin?C.emeraldSoft:outcome==='SL'?C.roseSoft:C.goldSoft;
    var iconText=isWin?'\u2713':outcome==='SL'?'\u2715':'\u2014';
    var resultColor=isWin?C.emerald:outcome==='SL'?C.rose:C.gold;
    var tagsHtml='';
    ...
    var notesHtml=e.notes?'<div style="font-size:11.5px;color:#B0BAB6;margin-top:5px;line-height:1.4">'+e.notes+'</div>':'';
  ...
  for(var i=0;i<filterChips.length;i++){
    ...
    chipsHtml+='<span onclick="setJournalFilter(\''+filterChips[i]+'\')" style="font-size:10px;font-weight:600;padding:5px 12px;border-radius:99px;cursor:pointer;background:'+(active?C.emeraldSoft:'rgba(255,255,255,0.03)')+';color:'+(active?C.emerald:C.text2)+';border:0.5px solid '+(active?C.emerald+'33':'rgba(255,255,255,0.05)')+'">'+label+'</span>';
  }

  return '<div class="section-h" style="color:'+C.white+'">Journal'+
    '<span class="sub" style="display:flex;gap:6px">'+
    '<span onclick="toggleEntryForm()" style="font-size:16px;font-weight:800;color:'+C.emerald+';cursor:pointer;line-height:1">+</span>'+
    '<span onclick="window.open(withCode(\'/api/journal\'),\'_blank\')" style="font-size:10px;font-weight:600;color:'+C.text2+';cursor:pointer;line-height:1.6">Export</span>'+
    '</span></div>'+
    '<div class="card" style="padding:16px 20px;border-color:'+C.emeraldBorder+';animation-delay:0s">'+
    '<div style="display:flex;justify-content:space-around">'+
    '<div style="text-align:center"><div class="count-up" style="font-size:22px;font-weight:800;color:'+C.emerald+';letter-spacing:-0.5px" data-target="'+wr+'">0</div><div style="font-size:9px;color:'+C.text2+';font-weight:500;margin-top:2px">Win Rate</div></div>'+
    '<div style="text-align:center"><div class="count-up" style="font-size:22px;font-weight:800;color:'+C.gold+';letter-spacing:-0.5px" data-target="'+(totalR||0)+'" data-dur="800">0</div><div style="font-size:9px;color:'+C.text2+';font-weight:500;margin-top:2px">Total R</div></div>'+
    '<div style="text-align:center"><div class="count-up" style="font-size:22px;font-weight:800;letter-spacing:-0.5px" data-target="'+entries.length+'">0</div><div style="font-size:9px;color:'+C.text2+';font-weight:500;margin-top:2px">Trades</div></div>'+
    '<div style="text-align:center"><div class="count-up" style="font-size:22px;font-weight:800;color:'+C.rose+';letter-spacing:-0.5px" data-target="'+losses.length+'">0</div><div style="font-size:9px;color:'+C.text2+';font-weight:500;margin-top:2px">Losses</div></div></div></div>'+
    ...
    '<div style="font-size:20px;font-weight:800;color:'+C.emerald+';letter-spacing:-0.5px;margin-top:2px">'+(streakInfo.winStreak??0)+'</div></div>'+
    ...
    '<div style="font-size:20px;font-weight:800;color:'+C.rose+';letter-spacing:-0.5px;margin-top:2px">'+(streakInfo.lossStreak??0)+'</div></div>'+
    '<div style="width:0.5px;background:rgba(255,255,255,0.05)"></div>'+
    '<div style="text-align:center"><div style="font-size:9px;color:'+C.text2+';font-weight:500">\uD83C\uDFC6 Best Day</div>'+
    '<div style="font-size:20px;font-weight:800;color:'+C.gold+';letter-spacing:-0.5px;margin-top:2px">'+(state.stats?.bestTrade?'+'+state.stats.bestTrade.rMultiple+'R':'--')+'</div></div></div>'+
...
function journalEntryForm(){
  ...
  for(var i=0;i<tagOpts.length;i++){
    ...
    tagsHtml+='<span onclick="toggleTag(this,\''+tagOpts[i]+'\')" style="font-size:9px;padding:4px 10px;border-radius:99px;cursor:pointer;background:'+(active?C.emeraldSoft:'rgba(255,255,255,0.03)')+';color:'+(active?C.emerald:C.text2)+';border:0.5px solid '+(active?C.emerald+'33':'rgba(255,255,255,0.05)')+'">'+tagOpts[i]+'</span>';
  }
  ...
  return '<div class="card" style="border-color:'+C.emeraldBorder+';animation-delay:0s">'+
    ...
    deleteHtml+
    '<div id="jStatus" style="font-size:11px;color:'+C.emerald+';margin-top:8px;text-align:center"></div></div>';
}

var selectedTags=[];
function toggleTag(el,tag){
  var idx=selectedTags.indexOf(tag);
  if(idx>-1){selectedTags.splice(idx,1);el.style.background='rgba(255,255,255,0.03)';el.style.color=C.text2;}
  else{selectedTags.push(tag);el.style.background=C.emeraldSoft;el.style.color=C.emerald;el.style.borderColor=C.emerald+'33';}
}
...
function calendarScreen(){
  ...
  for(var i=0;i<countries.length;i++){
    ...
    countryChips+='<span onclick="setCalFilter(\'country\',\''+countries[i]+'\')" style="font-size:9px;font-weight:600;padding:4px 12px;border-radius:99px;cursor:pointer;background:'+(active?C.emeraldSoft:'rgba(255,255,255,0.03)')+';color:'+(active?C.emerald:C.text2)+';border:0.5px solid '+(active?C.emerald+'33':'rgba(255,255,255,0.05)')+'">'+label+'</span>';
  }
  var impacts=['all','High','Medium','Low'];
  var impactChips='';
  for(var i=0;i<impacts.length;i++){
    var active=calFilter.impact===impacts[i];
    var colors={'High':C.rose,'Medium':C.gold,'Low':C.emerald};
    var dims={'High':C.roseSoft,'Medium':C.goldSoft,'Low':C.emeraldSoft};
    var c=impacts[i]==='all'?C.text2:colors[impacts[i]];
    var d=impacts[i]==='all'?'rgba(255,255,255,0.03)':dims[impacts[i]];
    impactChips+='<span onclick="setCalFilter(\'impact\',\''+impacts[i]+'\')" style="font-size:9px;font-weight:600;padding:4px 12px;border-radius:99px;cursor:pointer;background:'+(active?d:'rgba(255,255,255,0.03)')+';color:'+(active?c:C.text2)+';border:0.5px solid '+(active?c+'33':'rgba(255,255,255,0.05)')+'">'+impacts[i]+'</span>';
  }
  var filtered=events;
  ...
  for(var i=0;i<dayNames.length;i++){
    ...
    dateStrip+='<div style="text-align:center;padding:6px 14px;border-radius:99px;cursor:pointer;background:'+(active?C.emeraldSoft:'transparent')+';border:0.5px solid '+(active?C.emerald+'33':'rgba(255,255,255,0.04)')+'">'+
      '<div style="font-size:9px;color:'+(active?C.emerald:C.text2)+'">'+dayNames[i].toUpperCase()+'</div>'+
      '<div style="font-size:15px;font-weight:800;color:'+(active?C.emerald:C.white)+'">'+dt.getDate()+'</div></div>';
  }
  var eventsHtml='';
  if(filtered.length){
    for(var i=0;i<filtered.length;i++){
      var e=filtered[i];
      var impactColor=e.impact==='High'?C.rose:e.impact==='Medium'?C.gold:C.emerald;
      ...
      if(e.previous||e.forecast||e.actual){
        ...
        if(e.actual)dataRow+='<span>Actual: <b style="color:'+C.emerald+'">'+e.actual+'</b></span>';
        dataRow+='</div>';
      }
      var impBars='';
      ...
      eventsHtml+='<div class="card" style="padding:10px 16px;animation-delay:'+(i*0.03)+'s">'+
        '<div style="display:flex;justify-content:space-between;align-items:center">'+
        '<div style="font-size:9.5px;font-weight:600;color:'+impactColor+';letter-spacing:0.2px">\uD83D\uDD34'.repeat(Math.min(3,bars))+' '+(e.impact||'N/A')+' \u00b7 '+timeStr+
        (countdownStr?'<span style="margin-left:6px;font-size:9px;color:'+C.gold+';font-weight:700">\u23F1 '+countdownStr+'</span>':'')+
        '</div>'+
        (isReleased?'<span style="font-size:8px;font-weight:700;padding:2px 8px;border-radius:99px;background:'+C.emeraldSoft+';color:'+C.emerald+'">Released</span>':'')+
        '</div>'+
        '<div style="font-size:13px;font-weight:700;color:'+C.white+';margin-top:3px;line-height:1.3">'+(e.title||e.country||'')+'</div>'+
        dataRow+
        (e.description?'<div style="font-size:11px;color:#B0BAB6;margin-top:3px;line-height:1.4">'+e.description+'</div>':'')+
  ...
  return '<div style="display:flex;justify-content:space-between;align-items:baseline">'+
    ...
    '<span onclick="fetchAll()" style="font-size:9px;color:'+C.emerald+';cursor:pointer">\u21BB Refresh</span></div>'+
    ...
    '<div style="display:flex;justify-content:space-between;padding:8px 0"><span style="font-size:12px;font-weight:600;color:'+C.white+'">Upcoming High</span><span style="font-size:12px;font-weight:700;color:'+C.rose+'">'+high.length+'</span></div>'+
    '<div style="display:flex;justify-content:space-between;padding:8px 0;border-top:0.5px solid rgba(255,255,255,0.03)"><span style="font-size:12px;font-weight:600;color:'+C.white+'">Upcoming Medium</span><span style="font-size:12px;font-weight:700;color:'+C.gold+'">'+med.length+'</span></div>'+
    '<div style="display:flex;justify-content:space-between;padding:8px 0;border-top:0.5px solid rgba(255,255,255,0.03)"><span style="font-size:12px;font-weight:600;color:'+C.white+'">Upcoming Low</span><span style="font-size:12px;font-weight:700;color:'+C.text2+'">'+low.length+'</span></div></div>';
}
...
function settingsScreen(){
  ...
  return '<div class="section-h" style="color:'+C.white+'">Settings</div>'+
    ...
    '<div style="width:44px;height:44px;border-radius:99px;background:'+C.emeraldSoft+';display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:800;color:'+C.emerald+';flex-shrink:0">S</div>'+
    ...
    '<div style="font-size:11px;font-weight:700;color:#B0BAB6;padding:6px 4px 4px;letter-spacing:0.5px;text-transform:uppercase">Notifications</div>'+
    ...
    notifToggles+'</div>'+
    '<div style="font-size:11px;font-weight:700;color:#B0BAB6;padding:6px 4px 4px;letter-spacing:0.5px;text-transform:uppercase">About</div>'+
    '<div class="card" style="padding:4px 16px;animation-delay:0.1s">'+
    '<div class="setting-row" style="border-bottom:none"><div><div class="label" style="color:'+C.white+'">Version</div><div class="hint">Build '+new Date().toISOString().slice(0,10)+'</div></div>'+
    '<span style="font-size:13px;font-weight:600;color:'+C.text2+'">v10.0</span></div></div>'+
    ...
    '<span onclick="localStorage.clear();location.reload()" style="font-size:12px;font-weight:600;color:'+C.rose+';cursor:pointer">Clear</span></div></div>'+
    '<div style="text-align:center;padding:16px 0 8px;font-size:10px;color:#3D4C4A">Made with intent \u00b7 Slayers Trading</div>';
}
...
function newsScreen(){
  ...
  for(var i=0;i<categories.length;i++){
    ...
    var catColors={'Forex':C.emerald,'Economy':C.gold,'Geopolitics':C.rose,'Commodities':'#E8A87C'};
    var catDims={'Forex':C.emeraldSoft,'Economy':C.goldSoft,'Geopolitics':C.roseSoft,'Commodities':'rgba(232,168,124,0.08)'};
    var c=categories[i]==='All'?C.text2:catColors[categories[i]];
    var d=categories[i]==='All'?'rgba(255,255,255,0.03)':catDims[categories[i]];
    catChips+='<span onclick="setNewsFilter(\''+categories[i]+'\')" style="font-size:9px;font-weight:600;padding:4px 12px;border-radius:99px;cursor:pointer;background:'+(active?d:'rgba(255,255,255,0.03)')+';color:'+(active?c:C.text2)+';border:0.5px solid '+(active?c+'33':'rgba(255,255,255,0.05)')+'">'+categories[i]+'</span>';
  }
  var filtered=newsFilter==='All'?articles:articles.filter(function(a){return a.category===newsFilter;});
  var heroArticle=filtered[0];
  var heroHtml='';
  if(heroArticle){
    var heroCatColor={'Forex':C.emerald,'Economy':C.gold,'Geopolitics':C.rose,'Commodities':'#E8A87C'}[heroArticle.category]||C.text2;
    heroHtml='<div class="card" style="padding:0;overflow:hidden;border-color:'+heroCatColor+'1E;animation-delay:0s">'+
      '<div style="height:140px;background:linear-gradient(135deg,rgba(15,21,26,0.72),#1A2630);display:flex;align-items:flex-end;position:relative">'+
      ...
      '<div style="font-size:15px;font-weight:800;color:'+C.white+';margin-top:4px;line-height:1.3">'+heroArticle.title+'</div></div></div></div>';
  }
  var articlesHtml='';
  if(filtered.length>1){
    for(var i=1;i<filtered.length;i++){
      var a=filtered[i];
      var catColor={'Forex':C.emerald,'Economy':C.gold,'Geopolitics':C.rose,'Commodities':'#E8A87C'}[a.category]||C.text2;
      articlesHtml+='<div class="card" style="padding:12px 16px;cursor:pointer;animation-delay:'+(i*0.03)+'s">'+
        ...
        '<div style="font-size:13px;font-weight:700;color:'+C.white+';line-height:1.3">'+a.title+'</div>'+
        (a.summary?'<div style="font-size:11px;color:#B0BAB6;margin-top:4px;line-height:1.4">'+a.summary+'</div>':'')+
        '</div></div></div>';
    }
  }
  if(!articlesHtml&&!heroArticle)articlesHtml='<div class="card" style="text-align:center;padding:30px;color:'+C.text2+';font-size:12px">No news available.</div>';
  return '<div class="section-h" style="padding-top:12px;margin-bottom:8px;color:'+C.white+'">News \u2014 Market Intel</div>'+
...
function activeTradeWidget(t){
  ...
  var badges='';
  if(t.tp1Fired)badges+='<span style="font-size:8px;font-weight:600;padding:2px 8px;border-radius:99px;background:'+C.emeraldSoft+';color:'+C.emerald+'">TP1 \u2713</span>';
  if(t.beFired)badges+='<span style="font-size:8px;font-weight:600;padding:2px 8px;border-radius:99px;background:'+C.roseSoft+';color:'+C.rose+'">BE \u2713</span>';
  if(t.trailActive)badges+='<span style="font-size:8px;font-weight:600;padding:2px 8px;border-radius:99px;background:'+C.goldSoft+';color:'+C.gold+'">Trailing</span>';
  var duration='';
  ...
  return '<div class="card-glass" style="animation-delay:0s">'+
    ...
    '<span style="margin-left:auto;font-size:9px;font-weight:600;padding:3px 10px;border-radius:99px;background:'+(isB?C.emeraldSoft:C.roseSoft)+';color:'+(isB?C.emerald:C.rose)+';border:0.5px solid '+(isB?C.emerald:C.rose)+'33;text-transform:uppercase">'+(isB?'BUY':'SELL')+'</span></div>'+
    ...
    '<div style="text-align:center"><div style="font-size:10px;color:'+C.text2+'">Now</div><div style="font-size:14px;font-weight:800;color:'+C.emerald+';letter-spacing:-0.3px">'+(t.tp1||fmt(t.sl))+'</div></div>'+
    '<div style="text-align:right"><div style="font-size:10px;color:'+C.text2+'">TP1</div><div style="font-size:14px;font-weight:800;color:'+C.gold+';letter-spacing:-0.3px">'+(t.tp1?fmt(t.tp1):'--')+'</div></div></div>'+
    '<div class="progress-track" style="margin-top:8px"><div class="progress-fill" style="animation:fillBar 0.8s cubic-bezier(0.16,1,0.3,1);width:'+progress+'%;background:linear-gradient(90deg,'+C.emerald+','+C.gold+')"></div></div>'+
    '<div style="display:flex;justify-content:space-between;margin-top:10px;align-items:center">'+
    '<div style="display:flex;gap:6px">'+badges+'</div>'+
    '<div style="display:flex;align-items:center;gap:8px">'+
    '<span style="font-size:13px;font-weight:800;color:'+C.emerald+'">+'+rVal+'R</span>'+
...
function render(){
  ...
  for(var i=0;i<tabsArr.length;i++){
    ...
    var color=active?C.emerald:C.text2;
  ...
  app.innerHTML='<div style="display:flex;flex-direction:column;height:100dvh;background:'+C.bg+';color:'+C.white+';position:relative">'+
    ...
    '<div style="flex:1;overflow-y:auto;padding:calc(constant(safe-area-inset-top,0px) + 8px) 16px 10px;padding:calc(env(safe-area-inset-top,0px) + 8px) 16px 10px;background:'+C.bg+';color:'+C.white+'" id="content">'+skeletonHtml+'</div>'+
    '<div id="tabbar" style="flex-shrink:0;display:flex;position:relative;border-top:0.5px solid '+C.tabBorder+';background:#0A0E12;padding:6px 0 calc(6px + constant(safe-area-inset-bottom,0px));padding:6px 0 calc(6px + env(safe-area-inset-bottom,0px));z-index:10">'+
    '<div class="nav-indicator" style="left:'+(tabsArr.indexOf(state.tab)*20+10)+'%;width:20%"></div>'+
    tabBtns+'</div>'+
...
function marketPulseRow(){
  ...
  for(var i=0;i<maxChips;i++){
    ...
    var bg=p.signalDir!=='NONE'?(p.signalDir==='BULLISH'?C.emeraldSoft:C.roseSoft):'rgba(255,255,255,0.02)';
...
window.openDetail=function(id){for(var i=0;i<state.signals.length;i++)if(state.signals[i].id===id){state.selected=state.signals[i];break;}render();window.scrollTo(0,0);};
...
function renderLogin(errorMsg){
  ...
  app.innerHTML='<div style="min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:calc(24px + env(safe-area-inset-top)) 24px calc(24px + env(safe-area-inset-bottom));text-align:center;background:'+C.bg+';color:'+C.white+'">'+
    ...
    '<div class="card" style="width:100%;max-width:300px;padding:24px;border-color:'+C.emeraldBorder+'">'+
    '<div style="font-size:14px;font-weight:700;color:'+C.white+';margin-bottom:10px">Enter your access code</div>'+
