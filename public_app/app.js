...
'.nav-indicator{position:absolute;top:-0.5px;height:2px;border-radius:99px;background:#3ECF8E;transition:left 0.35s cubic-bezier(0.16,1,0.3,1),width 0.35s cubic-bezier(0.16,1,0.3,1);box-shadow:0 0 20px rgba(62,207,142,0.2)}'+
...
'@keyframes pulseRing{0%{box-shadow:0 0 0 0 rgba(62,207,142,0.5)}70%{box-shadow:0 0 0 8px rgba(62,207,142,0)}100%{box-shadow:0 0 0 0 rgba(62,207,142,0)}}'+
...
'.ptr-logo{width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,#3ECF8E,#0D9488);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:15px;color:#0A0E12;flex-shrink:0;box-shadow:0 0 20px rgba(62,207,142,0.15)}'+
...
function pill(text, color, dim, cls) {
  return '<span class="pill' + (cls ? ' ' + cls : '') + '" style="color:' + color + ';background:' + dim + ';border:0.5px solid ' + color + '33">' + text + '</span>';
}
...
function setupPTR(sc){
  ...
  sc.addEventListener('touchmove', function(e){
    ...
    if (pl) {
      pl.style.transform = 'rotate(' + (progress * 360) + 'deg)';
      pl.style.opacity = 0.4 + progress * 0.6;
    }
    if (pt) {
      pt.style.opacity = progress;
      pt.textContent = damped >= 60 ? 'Release to refresh' : 'Pull to refresh';
    }
  }, {passive:false});
  ...
  sc.addEventListener('touchend', function(e){
    ...
    if (ptr.curr >= 60) {
      ...
      setTimeout(function(){
        ...
        var pw2 = document.getElementById('ptrWrap');
        var pl2 = document.getElementById('ptrLogo');
        var pt2 = document.getElementById('ptrText');
...
async function fetchAll() {
  ...
  try {
    const [sigRes, activeRes, confluRes, statsRes, myStatsRes, journalRes, newsRes, newsFeedRes, settingsRes, tradeHistRes] = await withTimeout(Promise.all([
...
function statsOverview() {
  ...
  return '<div style="display:flex;gap:8px;margin-bottom:14px">' +
    ...
    '<div class="count-up" style="font-size:22px;font-weight:800;color:' + MINT + ';margin-top:2px;letter-spacing:-0.5px" data-target="' + (st.winRate || 0) + '">0</div></div>' +
    ...
    '<div class="count-up" style="font-size:22px;font-weight:800;color:' + GOLD + ';margin-top:2px;letter-spacing:-0.5px" data-target="' + (st.totalR || 0) + '" data-dur="800">0</div></div>' +
...
function journalScreen() {
  ...
  return     '<div class="section-h">Journal' +
    ...
    '<div style="text-align:center"><div class="count-up" style="font-size:22px;font-weight:800;color:' + MINT + ';letter-spacing:-0.5px" data-target="' + wr + '">0</div><div style="font-size:9px;color:' + GREY + ';font-weight:500;margin-top:2px">Win Rate</div></div>' +
    '<div style="text-align:center"><div class="count-up" style="font-size:22px;font-weight:800;color:' + GOLD + ';letter-spacing:-0.5px" data-target="' + (totalR || 0) + '" data-dur="800">0</div><div style="font-size:9px;color:' + GREY + ';font-weight:500;margin-top:2px">Total R</div></div>' +
    '<div style="text-align:center"><div class="count-up" style="font-size:22px;font-weight:800;letter-spacing:-0.5px" data-target="' + entries.length + '">0</div><div style="font-size:9px;color:' + GREY + ';font-weight:500;margin-top:2px">Trades</div></div>' +
     '<div style="text-align:center"><div class="count-up" style="font-size:22px;font-weight:800;color:' + RED + ';letter-spacing:-0.5px" data-target="' + losses.length + '">0</div><div style="font-size:9px;color:' + GREY + ';font-weight:500;margin-top:2px">Losses</div></div></div></div>' +
     equityChart(entries) +
     rBarChart(entries) +
...
function calendarScreen() {
  ...
  // Build country list from events
  var countries = ['all'];
  for (var i = 0; i < events.length; i++) {
    if (events[i].country && countries.indexOf(events[i].country) === -1) countries.push(events[i].country);
  }
  var countryChips = '';
  for (var i = 0; i < countries.length; i++) {
    var active = calFilter.country === countries[i];
    var label = countries[i] === 'all' ? 'All' : countries[i];
    countryChips += '<span onclick="setCalFilter(\'country\',\'' + countries[i] + '\')" style="font-size:9px;font-weight:600;padding:4px 12px;border-radius:99px;cursor:pointer;background:' + (active ? MINT_DIM : 'rgba(255,255,255,0.03)') + ';color:' + (active ? MINT : GREY) + ';border:0.5px solid ' + (active ? MINT + '33' : 'rgba(255,255,255,0.05)') + '">' + label + '</span>';
  }
  ...
  for (var i = 0; i < impacts.length; i++) {
    ...
    var colors = { 'High': RED, 'Medium': GOLD, 'Low': MINT };
    var dims = { 'High': RED_DIM, 'Medium': GOLD_DIM, 'Low': MINT_DIM };
    var c = impacts[i] === 'all' ? GREY : colors[impacts[i]];
    var d = impacts[i] === 'all' ? 'rgba(255,255,255,0.03)' : dims[impacts[i]];
    impactChips += '<span onclick="setCalFilter(\'impact\',\'' + impacts[i] + '\')" style="font-size:9px;font-weight:600;padding:4px 12px;border-radius:99px;cursor:pointer;background:' + (active ? d : 'rgba(255,255,255,0.03)') + ';color:' + (active ? c : GREY) + ';border:0.5px solid ' + (active ? c + '33' : 'rgba(255,255,255,0.05)') + '">' + impacts[i] + '</span>';
  }
  ...
  if (filtered.length) {
    for (var i = 0; i < filtered.length; i++) {
      var e = filtered[i];
      var impactColor = e.impact === 'High' ? RED : e.impact === 'Medium' ? GOLD : MINT;
      ...
      var diffMs = eventDate ? eventDate - now : 0;
      var countdownStr = '';
      ...
      if (e.previous || e.forecast || e.actual) {
        dataRow = '<div style="display:flex;gap:14px;margin-top:4px;font-size:10px;color:' + GREY + '">';
        if (e.previous) dataRow += '<span>Prev: <b style="color:' + WHITE + '">' + e.previous + '</b></span>';
        if (e.forecast) dataRow += '<span>Forecast: <b style="color:' + WHITE + '">' + e.forecast + '</b></span>';
        if (e.actual) dataRow += '<span>Actual: <b style="color:' + MINT + '">' + e.actual + '</b></span>';
        dataRow += '</div>';
      }
      var impBars = '';
      ...
      eventsHtml += '<div class="card" style="padding:10px 16px;animation-delay:' + (i * 0.03) + 's">' +
        ...
        (countdownStr ? '<span style="margin-left:6px;font-size:9px;color:' + GOLD + ';font-weight:700">\u23F1 ' + countdownStr + '</span>' : '') +
        '</div>' +
        (isReleased ? '<span style="font-size:8px;font-weight:700;padding:2px 8px;border-radius:99px;background:' + MINT_DIM + ';color:' + MINT + '">Released</span>' : '') +
        '</div>' +
        '<div style="font-size:13px;font-weight:700;margin-top:3px;line-height:1.3">' + (e.title || e.country || '') + '</div>' +
        dataRow +
        (e.description ? '<div style="font-size:11px;color:' + TEXT2 + ';margin-top:3px;line-height:1.4">' + e.description + '</div>' : '') +
        '<div style="display:flex;gap:3px;margin-top:5px">' + impBars + '</div></div>';
    }
  } else {
    eventsHtml = '<div class="card" style="text-align:center;padding:30px;color:' + GREY + ';font-size:12px">No events match the selected filters.</div>';
  }

  return '<div style="display:flex;justify-content:space-between;align-items:baseline">' +
    '<div class="section-h" style="padding-top:12px;margin-bottom:8px">Calendar</div>' +
    '<span onclick="fetchAll()" style="font-size:9px;color:' + MINT + ';cursor:pointer">\u21BB Refresh</span></div>' +
    ...
    '<div style="display:flex;justify-content:space-between;padding:8px 0"><span style="font-size:12px;font-weight:600">Upcoming High</span><span style="font-size:12px;font-weight:700;color:' + RED + '">' + high.length + '</span></div>' +
    '<div style="display:flex;justify-content:space-between;padding:8px 0;border-top:0.5px solid rgba(255,255,255,0.03)"><span style="font-size:12px;font-weight:600">Upcoming Medium</span><span style="font-size:12px;font-weight:700;color:' + GOLD + '">' + med.length + '</span></div>' +
    '<div style="display:flex;justify-content:space-between;padding:8px 0;border-top:0.5px solid rgba(255,255,255,0.03)"><span style="font-size:12px;font-weight:600">Upcoming Low</span><span style="font-size:12px;font-weight:700;color:' + GREY + '">' + low.length + '</span></div></div>';
}
...
function newsScreen() {
  ...
  for (var i = 0; i < categories.length; i++) {
    ...
    var catColors = { 'Forex': MINT, 'Economy': GOLD, 'Geopolitics': RED, 'Commodities': '#E8A87C' };
    var catDims = { 'Forex': MINT_DIM, 'Economy': GOLD_DIM, 'Geopolitics': RED_DIM, 'Commodities': 'rgba(232,168,124,0.08)' };
    var c = categories[i] === 'All' ? GREY : catColors[categories[i]];
    var d = categories[i] === 'All' ? 'rgba(255,255,255,0.03)' : catDims[categories[i]];
    catChips += '<span onclick="setNewsFilter(\'' + categories[i] + '\')" style="font-size:9px;font-weight:600;padding:4px 12px;border-radius:99px;cursor:pointer;background:' + (active ? d : 'rgba(255,255,255,0.03)') + ';color:' + (active ? c : GREY) + ';border:0.5px solid ' + (active ? c + '33' : 'rgba(255,255,255,0.05)') + '">' + categories[i] + '</span>';
  }

  var filtered = newsFilter === 'All' ? articles : articles.filter(function(a) { return a.category === newsFilter; });

  // Hero featured card (most recent)
  var heroArticle = filtered[0];
  var heroHtml = '';
  if (heroArticle) {
    var heroCatColor = { 'Forex': MINT, 'Economy': GOLD, 'Geopolitics': RED, 'Commodities': '#E8A87C' }[heroArticle.category] || GREY;
    heroHtml = '<div class="card" style="padding:0;overflow:hidden;border-color:' + heroCatColor + '1E;animation-delay:0s">' +
      '<div style="height:140px;background:linear-gradient(135deg,#0F151A,#1A2630);display:flex;align-items:flex-end;position:relative">' +
      '<div style="position:absolute;top:10px;right:10px">' +
      pill('Featured', heroCatColor, heroCatColor + '22') + '</div>' +
      '<div style="padding:16px;width:100%">' +
      '<span style="font-size:9px;color:' + GREY + '">' + heroArticle.source + ' \u00b7 ' + timeAgo(heroArticle.time) + '</span>' +
      '<div style="font-size:15px;font-weight:800;margin-top:4px;line-height:1.3">' + heroArticle.title + '</div></div></div></div>';
  }
  ...
  if (filtered.length > 1) {
    for (var i = 1; i < filtered.length; i++) {
      var a = filtered[i];
      var catColor = { 'Forex': MINT, 'Economy': GOLD, 'Geopolitics': RED, 'Commodities': '#E8A87C' }[a.category] || GREY;
      articlesHtml += '<div class="card" style="padding:12px 16px;cursor:pointer;animation-delay:' + (i * 0.03) + 's">' +
        ...
        '<span style="font-size:9px;color:' + GREY + '">' + timeAgo(a.time) + '</span></div>' +
        '<div style="font-size:13px;font-weight:700;line-height:1.3">' + a.title + '</div>' +
        (a.summary ? '<div style="font-size:11px;color:' + TEXT2 + ';margin-top:4px;line-height:1.4">' + a.summary + '</div>' : '') +
        '</div></div></div>';
    }
  }

  if (!articlesHtml && !heroArticle) {
    articlesHtml = '<div class="card" style="text-align:center;padding:30px;color:' + GREY + ';font-size:12px">No news available.</div>';
  }

  return '<div class="section-h" style="padding-top:12px;margin-bottom:8px">News \u2014 Market Intel</div>' +
    '<div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:12px">' + catChips + '</div>' +
    (heroArticle ? heroHtml : '') +
    articlesHtml;
}
...
function activeTradeWidget(t) {
  ...
  return '<div class="card-glass" style="animation-delay:0s">' +
    ...
    '<div class="progress-track" style="margin-top:8px"><div class="progress-fill" style="animation:fillBar 0.8s cubic-bezier(0.16,1,0.3,1);width:' + progress + '%;background:linear-gradient(90deg,' + MINT + ',' + GOLD + ')"></div></div>' +
...
function render() {
  ...
  app.innerHTML = '<div style="display:flex;flex-direction:column;height:100dvh;background:' + BLACK + ';color:' + WHITE + ';position:relative">' +
    ...
    '<div style="flex:1;overflow-y:auto;padding:calc(constant(safe-area-inset-top,0px) + 8px) 16px 10px;padding:calc(env(safe-area-inset-top,0px) + 8px) 16px 10px;background:' + BLACK + ';color:' + WHITE + '" id="content">' + skeletonHtml + '</div>' +
    '<div id="tabbar" style="flex-shrink:0;display:flex;position:relative;border-top:0.5px solid rgba(255,255,255,0.04);background:#0A0E12;padding:6px 0 calc(6px + constant(safe-area-inset-bottom,0px));padding:6px 0 calc(6px + env(safe-area-inset-bottom,0px));z-index:10">' +
    '<div class="nav-indicator" style="position:absolute;top:-0.5px;height:2px;border-radius:99px;background:' + MINT + ';transition:left 0.35s cubic-bezier(0.16,1,0.3,1),width 0.35s cubic-bezier(0.16,1,0.3,1);box-shadow:0 0 20px ' + MINT + '33"></div>' +
    tabBtns + '</div>' +
  ...
  if (indicator) { indicator.style.left = (idx * 20 + 10) + '%'; indicator.style.width = '20%'; }
