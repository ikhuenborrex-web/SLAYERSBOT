...
// ===== STATE =====
let state = { tab: 'signals', selected: null, signals: [], active: [], briefing: [], stats: null, myStats: null, loading: true, filter: { pair: '', dir: '', tf: '' }, showSettings: false, notifPrefs: {} };
...
async function fetchAll() {
  ...
  try {
    const [sigRes, activeRes, briefRes, statsRes, myStatsRes] = await withTimeout(Promise.all([
  ...
  return `
    ...
    ${state.briefing.map(p => `
      <div class="panel" style="display:flex;align-items:center;justify-content:space-between">
        <div><div style="color:${WHITE};font-weight:800;font-size:14.5px">${p.name || p.pair}</div><div class="mono" style="color:${GREY};font-size:11px">${p.price}</div></div>
...
function render() {
  ...
  app.innerHTML = `
    <div style="padding:calc(24px + env(safe-area-inset-top)) 16px 0">
      ...
      <div style="color:${GREY};font-size:11px;margin-top:4px;margin-bottom:18px">Signal Center · v8.2</div>
    ...
    <div style="flex:1;padding:18px 16px 0;overflow-y:auto" id="content"></div>
    <div style="position:relative;display:flex;border-top:1px solid ${BORDER};background:rgba(13,22,17,0.6);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);margin-top:16px">
      <div class="nav-underline" style="left:${['signals','briefing','stats'].indexOf(state.tab) * 33.333}%;width:33.333%"></div>
