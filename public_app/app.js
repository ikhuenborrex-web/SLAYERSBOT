...
'.pill{display:inline-flex;align-items:center;gap:4px;font-size:9px;font-weight:600;letter-spacing:0.3px;padding:3px 10px;border-radius:99px;text-transform:uppercase}'+
...
'.section-h .sub{font-size:11px;font-weight:500;color:#6B7A76;margin-left:auto}'+
...
'.toggle{width:42px;height:24px;border-radius:99px;background:#36413E;cursor:pointer;position:relative;flex-shrink:0;transition:background 0.3s}'+
'.toggle.on{background:#38BD82}'+
...
'.setting-row .hint{font-size:10.5px;color:#6B7A76;margin-top:2px;font-weight:400}'+
'@keyframes breathe{0%,100%{box-shadow:0 0 4px #38BD82;opacity:1}50%{box-shadow:0 0 14px #38BD82;opacity:0.4}}'+
'.live-dot{width:6px;height:6px;border-radius:99px;background:#38BD82;animation:breathe 2.4s ease-in-out infinite;flex-shrink:0}'+
'.tab-btn{flex:1;display:flex;flex-direction:column;align-items:center;gap:1px;padding:8px 0 2px;border:none;background:none;cursor:pointer;transition:color 0.3s;font-family:inherit;position:relative}'+
'@keyframes fadeUp{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}'+
'.nav-indicator{position:absolute;top:-0.5px;height:2px;border-radius:99px;background:#38BD82;transition:left 0.35s cubic-bezier(0.34,1.56,0.64,1),width 0.35s cubic-bezier(0.34,1.56,0.64,1);box-shadow:0 0 16px rgba(56,189,130,0.2)}'+
...
'.ember:nth-child(odd){width:1.5px;height:1.5px}'+
...
'.btn-primary{display:inline-flex;align-items:center;justify-content:center;gap:6px;padding:10px 20px;border-radius:99px;background:#38BD82;border:none;color:#070A0D;font-family:inherit;font-size:12px;font-weight:700;cursor:pointer;transition:all 0.2s}'+
'.btn-primary:active{transform:scale(0.96);opacity:0.9}'+
'.btn-outline{display:inline-flex;align-items:center;justify-content:center;gap:6px;padding:10px 20px;border-radius:99px;background:transparent;border:0.5px solid rgba(255,255,255,0.08);color:#6B7A76;font-family:inherit;font-size:12px;font-weight:600;cursor:pointer;transition:all 0.2s}'+
'.btn-outline:active{transform:scale(0.96)}';
document.head.appendChild(s);})();

const BLACK='#070A0D', PANEL='#0E151D', CARD='#0C1117', BORDER='rgba(56,189,130,0.08)',
      MINT='#38BD82', MINT_DIM='rgba(56,189,130,0.08)', MINT2='#2DA673',
      GOLD='#C8A34A', GOLD_DIM='rgba(200,163,74,0.08)',
      RED='#E16060', RED_DIM='rgba(225,96,96,0.08)',
      WHITE='#E8EDEA', TEXT2='#B0BAB6', GREY='#6B7A76', MUTED='#36413E';
...
let state = {
  ...
  journal: [], news: [], settings: null, notifPrefs: {},
  loading: true,
  filter: { pair: '', dir: '', tf: '' },
  journalFilter: 'all',
  showEntryForm: false,
...
async function fetchAll() {
  ...
  try {
    const [sigRes, activeRes, confluRes, statsRes, myStatsRes, journalRes, newsRes, settingsRes] = await withTimeout(Promise.all([
...
function signalCard(s) {
  ...
  const criteria = (s.criteria || []).map(c =>
    `<span style="font-size:10px;color:${MINT};background:${MINT_DIM};border:0.5px solid ${MINT}44;border-radius:99px;padding:3px 10px;font-weight:600;display:inline-block;margin:0 6px 6px 0">${c}</span>`
  ).join('');
  return `
  <div class="card" onclick="openDetail('${s.id}')" style="cursor:pointer;border-left:2.5px solid ${tierColor};animation-delay:${Math.random()*0.2}s">
...
function detailPage(s) {
  const isBuy = s.type === 'BULLISH' || s.type === 'BUY';
  const tierColor = s.tier === 'ELITE' ? MINT : GOLD;
  const criteriaList = (s.criteria || []).map(c =>
    `<div style="display:flex;align-items:center;gap:8px;padding:6px 0">${icon('check', MINT, 15)}<span style="font-size:13px">${c}</span></div>`
  ).join('');
  ...
  return `
  <div style="display:flex;flex-direction:column;height:100dvh;background:${BLACK}">
    <div style="flex:1;overflow-y:auto;background:${BLACK}">
      ...
      <div style="padding:8px 16px">
        ...
        <button onclick="toggleTrack('${s.id}', ${!!s.isTracked})" style="width:100%;background:${s.isTracked ? MINT_DIM : 'transparent'};border:0.5px solid ${s.isTracked ? MINT : BORDER};border-radius:99px;padding:14px 0;display:flex;align-items:center;justify-content:center;gap:8px;cursor:pointer;margin-bottom:12px">
          ...
          <span style="color:${s.isTracked ? MINT : GREY};font-weight:700;font-size:13px">${s.isTracked ? 'Tracking — you will get updates on this trade' : "I'm in this trade — notify me"}</span>
        </button>
  ...
  return state.confluence.map(p => {
    ...
    return `<div class="card" style="border-left:2.5px solid ${tc};animation-delay:${Math.random()*0.15}s">
      ...
      <div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:8px">
        ...
        ${pill(p.convictionLabel, tc, tc + '22')}
      </div>
      ...
      ${p.factors.length ? `<div style="display:flex;gap:4px;flex-wrap:wrap">${p.factors.map(f => `<span style="font-size:9px;color:${MINT};background:${MINT_DIM};border-radius:99px;padding:2px 8px">✓ ${f}</span>`).join('')}</div>` : ''}
    </div>`;
  }).join('');
}
  ...
  return `
    <div style="display:flex;gap:8px;margin-bottom:14px">
      <div class="card" style="flex:1;padding:12px;text-align:center;animation-delay:0s">
        ...
        <div style="font-size:22px;font-weight:800;color:${MINT};margin-top:2px;letter-spacing:-0.5px">${st.winRate ?? '--'}%</div>
      </div>
      <div class="card" style="flex:1;padding:12px;text-align:center;animation-delay:0.05s">
        ...
        <div style="font-size:22px;font-weight:800;color:${GOLD};margin-top:2px;letter-spacing:-0.5px">${(st.totalR >= 0 ? '+' : '') + (st.totalR ?? '--')}</div>
      </div>
      <div class="card" style="flex:1;padding:12px;text-align:center;animation-delay:0.1s">
        ...
        <div style="font-size:22px;font-weight:800;margin-top:2px;letter-spacing:-0.5px">${state.active.length}</div>
      </div>
    </div>`;
}
...
function journalScreen() {
  ...
  const wins = entries.filter(e => e.outcome === 'WIN' || e.outcome === 'TP1' || e.outcome === 'TP2');
  const losses = entries.filter(e => e.outcome === 'SL');
  const bes = entries.filter(e => e.outcome === 'BE');
  const wr = (wins.length + losses.length) ? Math.round((wins.length / (wins.length + losses.length)) * 100) : 0;
  const totalR = entries.reduce((a, e) => a + (e.rMultiple || 0), 0);
  const filtered = state.journalFilter === 'all' ? entries
    : state.journalFilter === 'wins' ? wins
    : state.journalFilter === 'losses' ? losses
    : entries.filter(e => e.outcome === state.journalFilter);
  ...
  for (let i = 28; i >= 0; i--) {
    ...
    const dayEntry = entries.find(e => e.createdAt && e.createdAt.slice(0, 10) === ds);
    heatmapDays.push({ date: ds, outcome: dayEntry ? dayEntry.outcome : null });
  }
  const heatmapHtml = heatmapDays.map(d => {
    const color = d.outcome === 'WIN' || d.outcome === 'TP1' || d.outcome === 'TP2' ? MINT
      : d.outcome === 'SL' ? RED
      : d.outcome === 'BE' ? GOLD
      : 'rgba(255,255,255,0.04)';
    return `<span style="display:inline-block;width:10px;height:10px;border-radius:2.5px;background:${color}"></span>`;
  }).join('');
  ...
  return `
    <div class="section-h">Journal</div>

    <!-- Stats bar -->
    <div class="card" style="padding:16px 20px;background:${CARD};border-color:rgba(56,189,130,0.12);animation-delay:0s">
      <div style="display:flex;justify-content:space-around">
        ...
        <div style="text-align:center"><div style="font-size:22px;font-weight:800;color:${RED};letter-spacing:-0.5px">${losses.length}</div><div style="font-size:9px;color:${GREY};font-weight:500;margin-top:2px">Losses</div></div>
      </div>
    </div>
    ...
    <div class="card" style="animation-delay:0.05s">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
        <span style="font-size:13px;font-weight:700">Trading Activity</span>
      ...
      <div style="display:flex;gap:3px;flex-wrap:wrap">${heatmapHtml}</div>
    </div>

    <!-- Streaks -->
    <div class="card" style="display:flex;justify-content:space-around;padding:14px 10px;animation-delay:0.1s">
      <div style="text-align:center">
        <div style="font-size:9px;color:${GREY};font-weight:500">🔥 Win Streak</div>
        <div style="font-size:20px;font-weight:800;color:${MINT};letter-spacing:-0.5px;margin-top:2px">${streakInfo.winStreak ?? 0}</div>
      </div>
      <div style="width:0.5px;background:rgba(255,255,255,0.05)"></div>
      <div style="text-align:center">
        <div style="font-size:9px;color:${GREY};font-weight:500">😤 Loss Streak</div>
        <div style="font-size:20px;font-weight:800;color:${RED};letter-spacing:-0.5px;margin-top:2px">${streakInfo.lossStreak ?? 0}</div>
      </div>
      <div style="width:0.5px;background:rgba(255,255,255,0.05)"></div>
      <div style="text-align:center">
        <div style="font-size:9px;color:${GREY};font-weight:500">🏆 Best Day</div>
        <div style="font-size:20px;font-weight:800;color:${GOLD};letter-spacing:-0.5px;margin-top:2px">${state.stats?.bestTrade ? '+' + state.stats.bestTrade.rMultiple + 'R' : '--'}</div>
      </div>
    </div>

    <!-- Add entry button -->
    <div style="display:flex;gap:8px;margin-bottom:12px">
      <button onclick="toggleEntryForm()" style="flex:1;background:${MINT};border:none;border-radius:99px;padding:12px 0;color:${BLACK};font-weight:800;font-size:12px;cursor:pointer">+ New Journal Entry</button>
      <button onclick="window.open(withCode('/api/journal'), '_blank')" class="btn-outline" style="flex:0">Export</button>
    </div>

    ${state.showEntryForm ? journalEntryForm() : ''}

    <!-- Filter chips -->
    <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:12px">
      ${['all', 'wins', 'losses', 'BE', ...tags].map(t => {
        const active = state.journalFilter === t;
        return `<span onclick="setJournalFilter('${t}')" style="font-size:10px;font-weight:600;padding:5px 12px;border-radius:99px;cursor:pointer;background:${active ? MINT_DIM : 'rgba(255,255,255,0.03)'};color:${active ? MINT : GREY};border:0.5px solid ${active ? 'rgba(56,189,130,0.2)' : 'rgba(255,255,255,0.05)'}">${t === 'all' ? 'All' : t.charAt(0).toUpperCase() + t.slice(1)}</span>`;
      }).join('')}
    </div>

    <!-- Journal entries -->
    ${filtered.length ? filtered.map((e, i) => `
      <div class="card" style="padding:12px 16px;animation-delay:${i*0.03}s">
        <div style="display:flex;gap:12px;align-items:flex-start">
          <div style="width:30px;height:30px;border-radius:99px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:13px;background:${e.outcome === 'WIN' || e.outcome === 'TP1' || e.outcome === 'TP2' ? MINT_DIM : e.outcome === 'SL' ? RED_DIM : GOLD_DIM};color:${e.outcome === 'WIN' || e.outcome === 'TP1' || e.outcome === 'TP2' ? MINT : e.outcome === 'SL' ? RED : GOLD}">${e.outcome === 'WIN' || e.outcome === 'TP1' || e.outcome === 'TP2' ? '✓' : e.outcome === 'SL' ? '✕' : '—'}</div>
          <div style="flex:1;min-width:0">
            <div style="display:flex;justify-content:space-between;align-items:center">
              <span style="font-weight:700;font-size:13px">${e.pair}</span>
              <span style="font-weight:800;font-size:13px;color:${e.outcome === 'WIN' || e.outcome === 'TP1' || e.outcome === 'TP2' ? MINT : e.outcome === 'SL' ? RED : GOLD}">${e.rMultiple >= 0 ? '+' : ''}${e.rMultiple || 0}R</span>
            </div>
            <div style="font-size:10px;color:${GREY};margin-top:2px">${e.direction || ''} · ${e.tf || ''} · ${e.duration || ''} · ${e.createdAt ? timeAgo(e.createdAt) : ''}</div>
            ${e.notes ? `<div style="font-size:11.5px;color:${TEXT2};margin-top:5px;line-height:1.4">${e.notes}</div>` : ''}
            ${e.tags && e.tags.length ? `<div style="display:flex;gap:4px;margin-top:5px">${e.tags.map(t => `<span style="font-size:8.5px;padding:2px 8px;border-radius:99px;background:${MINT_DIM};color:${MINT};border:0.5px solid rgba(56,189,130,0.1)">${t}</span>`).join('')}</div>` : ''}
          </div>
        </div>
      </div>
    `).join('') : `<div class="card" style="text-align:center;padding:30px;color:${GREY};font-size:12px">${state.journalFilter === 'all' ? 'No journal entries yet. Tap "+ New Journal Entry" to start tracking your trades.' : 'No entries match this filter.'}</div>`}
  `;
}

function journalEntryForm() {
  return `
    <div class="card" style="border-color:rgba(56,189,130,0.2);animation-delay:0s">
      <div style="font-size:13px;font-weight:700;margin-bottom:12px">New Journal Entry</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px">
        <input id="jPair" placeholder="Pair (e.g. EURUSD)" style="background:${BLACK};border:0.5px solid ${BORDER};border-radius:8px;padding:10px;color:${WHITE};font-size:12px;outline:none">
        ...
        <select id="jOutcome" style="background:${BLACK};border:0.5px solid ${BORDER};border-radius:8px;padding:10px;color:${WHITE};font-size:12px;outline:none">
          <option value="WIN">WIN</option><option value="TP1">TP1</option><option value="TP2">TP2</option>
          <option value="SL">SL</option><option value="BE">BE</option>
        </select>
        <input id="jR" type="number" step="0.1" placeholder="R multiple (e.g. 1.8)" style="background:${BLACK};border:0.5px solid ${BORDER};border-radius:8px;padding:10px;color:${WHITE};font-size:12px;outline:none">
        <input id="jDuration" placeholder="Duration (e.g. 2h 34m)" style="background:${BLACK};border:0.5px solid ${BORDER};border-radius:8px;padding:10px;color:${WHITE};font-size:12px;outline:none">
      </div>
      <textarea id="jNotes" placeholder="Notes about the trade..." style="width:100%;background:${BLACK};border:0.5px solid ${BORDER};border-radius:8px;padding:10px;color:${WHITE};font-size:12px;outline:none;resize:none;height:60px;font-family:inherit"></textarea>
      <div style="display:flex;gap:6px;flex-wrap:wrap;margin:10px 0">
        ${['Displacement', 'Liq Sweep', 'MSS', 'Eng. Liq', 'FVG', 'OB', 'CRT', 'Early Entry', 'Confident', 'Impatient', 'Neutral'].map(t =>
          `<span onclick="toggleTag(this, '${t}')" style="font-size:9px;padding:4px 10px;border-radius:99px;cursor:pointer;background:rgba(255,255,255,0.03);color:${GREY};border:0.5px solid rgba(255,255,255,0.05)">${t}</span>`
        ).join('')}
      </div>
      <div style="display:flex;gap:8px">
        <button onclick="saveJournalEntry()" class="btn-primary" style="flex:1">Save Entry</button>
        <button onclick="toggleEntryForm()" class="btn-outline" style="flex:0">Cancel</button>
      </div>
      <div id="jStatus" style="font-size:11px;color:${MINT};margin-top:8px;text-align:center"></div>
    </div>`;
}
...
function toggleTag(el, tag) {
  ...
  else { selectedTags.push(tag); el.style.background = MINT_DIM; el.style.color = MINT; el.style.borderColor = 'rgba(56,189,130,0.2)'; }
}

async function saveJournalEntry() {
  const pair = document.getElementById('jPair').value.trim().toUpperCase();
  const direction = document.getElementById('jDir').value;
  const tf = document.getElementById('jTF').value;
  const outcome = document.getElementById('jOutcome').value;
  const rMultiple = parseFloat(document.getElementById('jR').value) || 0;
  ...
  try {
    const res = await fetch(withCode('/api/journal'), {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ pair, direction, tf, outcome, rMultiple, duration, notes, tags: [...selectedTags] })
    });
    if (res.ok) {
      ...
      // Refetch journal
      const jRes = await fetch(withCode('/api/journal'));
...
window.setJournalFilter = (f) => { state.journalFilter = f; render(); };

// ===== RENDER: CALENDAR SCREEN =====
function calendarScreen() {
  const events = state.news;
  const high = events.filter(e => e.impact === 'High');
  const med = events.filter(e => e.impact === 'Medium' || e.impact === 'MEDIUM');
  const low = events.filter(e => e.impact === 'Low' || e.impact === 'LOW');

  return `
    <div class="section-h">Calendar</div>
    ...
    <div style="display:flex;gap:6px;overflow-x:auto;padding-bottom:4px;margin-bottom:14px;scrollbar-width:none">
      ${['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((d, i) => {
        ...
        const active = dt.toISOString().slice(0, 10) === new Date().toISOString().slice(0, 10);
        return `<div style="text-align:center;padding:6px 14px;border-radius:99px;cursor:pointer;background:${active ? MINT_DIM : 'transparent'};border:0.5px solid ${active ? 'rgba(56,189,130,0.2)' : 'rgba(255,255,255,0.04)'}">
          <div style="font-size:9px;color:${active ? MINT : GREY}">${d.toUpperCase()}</div>
          <div style="font-size:15px;font-weight:800;color:${active ? MINT : WHITE}">${dt.getDate()}</div>
        </div>`;
      }).join('')}
    </div>

    ${events.length ? events.map((e, i) => {
      const impactColor = e.impact === 'High' || e.impact === 'HIGH' ? RED : e.impact === 'Medium' || e.impact === 'MEDIUM' ? GOLD : MINT;
      const bars = e.impact === 'High' || e.impact === 'HIGH' ? 3 : e.impact === 'Medium' || e.impact === 'MEDIUM' ? 2 : 1;
      return `<div class="card" style="padding:10px 16px;animation-delay:${i*0.03}s">
        <div style="font-size:9.5px;font-weight:600;color:${impactColor};letter-spacing:0.2px">${'🔴'.repeat(Math.min(3, bars))} ${e.impact || 'N/A'} · ${e.date ? new Date(e.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--:--'}</div>
        ...
        <div style="display:flex;gap:3px;margin-top:5px">${Array.from({ length: 3 }, (_, j) => `<span style="width:12px;height:3px;border-radius:99px;background:${j < bars ? impactColor : 'rgba(255,255,255,0.05)'}"></span>`).join('')}</div>
      </div>`;
    }).join('') : `<div class="card" style="text-align:center;padding:30px;color:${GREY};font-size:12px">No upcoming events loaded.</div>`}
    ...
    <div class="card" style="margin-top:4px;animation-delay:0.1s">
      ...
      <div style="display:flex;justify-content:space-between;padding:8px 0">
        <span style="font-size:12px;font-weight:600">High Impact</span>
        <span style="font-size:12px;font-weight:700;color:${RED}">${high.length}</span>
      </div>
      <div style="display:flex;justify-content:space-between;padding:8px 0;border-top:0.5px solid rgba(255,255,255,0.03)">
        <span style="font-size:12px;font-weight:600">Medium Impact</span>
        <span style="font-size:12px;font-weight:700;color:${GOLD}">${med.length}</span>
      </div>
      <div style="display:flex;justify-content:space-between;padding:8px 0;border-top:0.5px solid rgba(255,255,255,0.03)">
        <span style="font-size:12px;font-weight:600">Low Impact</span>
        <span style="font-size:12px;font-weight:700;color:${GREY}">${low.length}</span>
      </div>
    </div>
  `;
}
...
function settingsScreen() {
  ...
  const notifToggles = notifItems.map(([label, key, hint]) => {
    const enabled = prefs[key] !== false;
    return `<div class="setting-row">
      <div><div class="label">${label}</div><div class="hint">${hint}</div></div>
      <div class="toggle ${enabled ? 'on' : ''}" onclick="toggleNotifPref('${key}', ${!enabled})"></div>
    </div>`;
  }).join('');
  ...
  return `
    ...
    <div class="card" style="display:flex;align-items:center;gap:14px;padding:14px 16px;animation-delay:0s">
      ...
      <div style="flex:1">
        <div style="font-size:14px;font-weight:700">Slayers Member</div>
        <div style="font-size:11px;color:${GREY};margin-top:1px">Code: ${getCode().slice(0, 8)}...</div>
      </div>
      <span style="font-size:18px;color:${GREY}">›</span>
    </div>
    ...
    <div class="card" style="padding:4px 16px;animation-delay:0.05s">
      <div class="setting-row">
        <div><div class="label">Push Notifications</div><div class="hint">Receive alerts on your phone</div></div>
      ...
      <div class="setting-row">
        <div><div class="label">Sound Alerts</div><div class="hint">Play sound on new signals</div></div>
    ...
    <!-- App Settings -->
    <div style="font-size:11px;font-weight:700;color:${TEXT2};padding:6px 4px 4px;letter-spacing:0.5px;text-transform:uppercase">App Settings</div>
    <div class="card" style="padding:4px 16px;animation-delay:0.1s">
      <div class="setting-row">
        <div><div class="label">Theme</div><div class="hint">Dark · System · Light</div></div>
        <span style="font-size:13px;font-weight:600;color:${MINT}">Dark ›</span>
      </div>
      <div class="setting-row">
        <div><div class="label">Chart Default TF</div><div class="hint">Default timeframe for charts</div></div>
        <span style="font-size:13px;font-weight:600;color:${GREY}">${s.defaultTF || '4H'} ›</span>
      </div>
      <div class="setting-row">
        <div><div class="label">Default Risk %</div><div class="hint">Per-trade risk allocation</div></div>
        <span style="font-size:13px;font-weight:600;color:${GREY}">${s.defaultRisk || 0.5}% ›</span>
      </div>
      <div class="setting-row" style="border-bottom:none">
        <div><div class="label">Quote Currency</div><div class="hint">Display P&L in</div></div>
        <span style="font-size:13px;font-weight:600;color:${GREY}">${s.quoteCurrency || 'USD'} ›</span>
      </div>
    </div>

    <!-- Data -->
    <div style="font-size:11px;font-weight:700;color:${TEXT2};padding:6px 4px 4px;letter-spacing:0.5px;text-transform:uppercase">Data & Privacy</div>
    <div class="card" style="padding:4px 16px;animation-delay:0.15s">
      <div class="setting-row">
        <div><div class="label">Export Journal</div><div class="hint">All your trade data as CSV</div></div>
        <span style="font-size:16px;color:${GREY}">↓</span>
      </div>
      <div class="setting-row" style="border-bottom:none">
        <div><div class="label">Clear Local Data</div><div class="hint">Reset app cache</div></div>
        <span onclick="localStorage.clear();location.reload()" style="font-size:12px;font-weight:600;color:${RED};cursor:pointer">Clear</span>
      </div>
    </div>
    ...
    <div class="card" style="padding:4px 16px;animation-delay:0.2s">
      <div class="setting-row" style="border-bottom:none">
        <div><div class="label">Version</div><div class="hint">Build ${new Date().toISOString().slice(0, 10)}</div></div>
        <span style="font-size:13px;font-weight:600;color:${GREY}">v10.0</span>
      </div>
    </div>
...
function render() {
  ...
  const tabLabels = { overview: 'Overview', journal: 'Journal', calendar: 'Calendar', settings: 'Settings' };
  app.innerHTML = `
    <div style="display:flex;flex-direction:column;height:100dvh;background:${BLACK}">
      <div style="flex:1;overflow-y:auto;padding:0 16px 10px;background:${BLACK}" id="content">
        ${state.loading ? '<div style="padding:40px 0"><div class="skeleton" style="height:200px;margin-bottom:14px"></div><div class="skeleton" style="height:200px"></div></div>' : ''}
      </div>
      <div id="tabbar" style="flex-shrink:0;display:flex;position:relative;border-top:0.5px solid rgba(255,255,255,0.04);background:rgba(7,10,13,0.9);backdrop-filter:blur(28px) saturate(1.5);-webkit-backdrop-filter:blur(28px) saturate(1.5);padding:6px 0 calc(6px + env(safe-area-inset-bottom,0px));z-index:10">
        <div class="nav-indicator" style="position:absolute;top:-0.5px;height:2px;border-radius:99px;background:${MINT};transition:left 0.35s cubic-bezier(0.34,1.56,0.64,1),width 0.35s cubic-bezier(0.34,1.56,0.64,1);box-shadow:0 0 16px rgba(56,189,130,0.2)"></div>
        ${['overview','journal','calendar','settings'].map(t => `
          <button class="tab-btn" data-tab="${t}" style="flex:1;display:flex;flex-direction:column;align-items:center;gap:1px;padding:8px 0 2px;border:none;background:none;cursor:pointer;color:${state.tab===t?MINT:MUTED};transition:color 0.3s;font-family:inherit;position:relative">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">${ICONS[t==='overview'?'grid':t==='journal'?'book':t==='calendar'?'cal':'gear']}</svg>
            <span style="font-size:9px;font-weight:${state.tab===t?600:500};letter-spacing:0.2px;margin-top:1px">${tabLabels[t]}</span>
          </button>
        `).join('')}
      </div>
    </div>`;
  // Position nav indicator
  const tabs = ['overview','journal','calendar','settings'];
...
function overviewScreen() {
  ...
  const activeTrade = hasActive ? state.active[0] : null;
  return `
    ${activeTrade ? activeTradeWidget(activeTrade) : ''}
    <div class="section-h" style="padding-top:4px">Market Pulse</div>
    ${marketPulseRow()}
    ${statsOverview()}
    ${state.signals.length ? `<div class="section-h">Recent Signals</div>` + state.signals.slice(0, 3).map(signalCard).join('') + (state.signals.length > 3 ? `<div style="text-align:center;padding:8px;color:${GREY};font-size:11px;cursor:pointer" onclick="setTab('journal')">View all ${state.signals.length} signals →</div>` : '') : ''}
    ${!state.signals.length && !hasActive ? emptyState(state.fetchError ? 'Connection problem: ' + state.fetchError : 'No signals yet. Waiting for the next scan...') : ''}
  `;
}
...
function activeTradeWidget(t) {
  ...
  const p = t.dec || 5;
  const progress = t.tp1 ? Math.min(100, Math.max(0, ((isB ? t.qmLevel - (t.tp1) : (t.tp1) - t.qmLevel) / (isB ? t.qmLevel - (t.tp1 || t.sl) : (t.tp1 || t.sl) - t.qmLevel)) * 100)) : 0;
  return `
    <div class="card" style="border:0.5px solid rgba(56,189,130,0.15);background:${CARD};margin-top:14px;animation-delay:0s">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px">
        <span class="live-dot" style="width:6px;height:6px;border-radius:99px;background:${MINT};animation:breathe 2.4s ease-in-out infinite;flex-shrink:0"></span>
        ...
        <span style="margin-left:auto;font-size:9px;font-weight:600;padding:3px 10px;border-radius:99px;background:${isB ? MINT_DIM : RED_DIM};color:${isB ? MINT : RED};border:0.5px solid ${isB ? MINT : RED}33;text-transform:uppercase">${isB ? 'BUY' : 'SELL'}</span>
      </div>
      <div style="display:flex;justify-content:space-between;margin-bottom:4px">
        ...
        <div style="text-align:right"><div style="font-size:10px;color:${GREY}">TP1</div><div style="font-size:14px;font-weight:800;color:${GOLD};letter-spacing:-0.3px">${t.tp1 ? fmt(t.tp1) : '--'}</div></div>
      </div>
      <div class="progress-track" style="margin-top:8px;height:3px;background:rgba(255,255,255,0.04);border-radius:99px;overflow:hidden">
        <div class="progress-fill" style="height:100%;border-radius:99px;width:${progress}%;background:linear-gradient(90deg,${MINT},${GOLD});transition:width 0.8s cubic-bezier(0.34,1.56,0.64,1)"></div>
      </div>
      <div style="display:flex;justify-content:space-between;margin-top:10px;align-items:center">
        <div style="display:flex;gap:6px">
          ${t.tp1Fired ? `<span style="font-size:8px;font-weight:600;padding:2px 8px;border-radius:99px;background:${MINT_DIM};color:${MINT};border:0.5px solid rgba(56,189,130,0.15)">TP1 ✓</span>` : ''}
          ${t.beFired ? `<span style="font-size:8px;font-weight:600;padding:2px 8px;border-radius:99px;background:${RED_DIM};color:${RED};border:0.5px solid rgba(225,96,96,0.15)">BE ✓</span>` : ''}
          ${t.trailActive ? `<span style="font-size:8px;font-weight:600;padding:2px 8px;border-radius:99px;background:rgba(200,163,74,0.08);color:${GOLD};border:0.5px solid rgba(200,163,74,0.15)">Trailing</span>` : ''}
        </div>
        <div style="display:flex;align-items:center;gap:8px">
          <span style="font-size:13px;font-weight:800;color:${MINT}">+${((t.tp1 ? Math.abs(t.tp1 - t.qmLevel) : 0) / Math.abs(t.qmLevel - t.sl)).toFixed(1) || '0.0'}R</span>
          <span style="font-size:10px;color:${GREY}">${t.openTime ? Math.floor((Date.now() - t.openTime) / 3600000) + 'h ' + Math.floor(((Date.now() - t.openTime) % 3600000) / 60000) + 'm' : ''}</span>
        </div>
      </div>
    </div>`;
}
...
function marketPulseRow() {
  const pairs = state.confluence.length ? state.confluence : [];
  const chips = pairs.slice(0, 7).map(p => {
    const c = p.signalDir === 'BULLISH' ? MINT : p.signalDir === 'BEARISH' ? RED : GREY;
    const bg = p.signalDir !== 'NONE' ? (p.signalDir === 'BULLISH' ? MINT_DIM : RED_DIM) : 'transparent';
    return `<span style="display:inline-flex;align-items:center;gap:5px;padding:6px 14px;border-radius:99px;border:0.5px solid ${c}44;background:${bg};color:${c};font-size:12px;font-weight:600;cursor:pointer;white-space:nowrap;transition:all 0.2s"
      onclick="setTab('journal');state.journalFilter='all';render()">
      <span style="width:5px;height:5px;border-radius:99px;background:${c}"></span>${p.name || p.id}
    </span>`;
  }).join('');
  return `<div style="display:flex;gap:8px;overflow-x:auto;padding-bottom:4px;margin-bottom:6px;scrollbar-width:none">${chips || '<span style="color:' + GREY + ';font-size:12px">Loading...</span>'}</div>`;
}

// ===== ACTIONS =====
window.setTab = (t) => { state.tab = t; state.selected = null; render(); };
...
document.addEventListener('click', (e) => {
  ...
  if (btn) {
    ...
    document.querySelectorAll('.tab-btn').forEach(b => b.style.color = MUTED);
    btn.style.color = MINT;
    state.tab = tab;
    render();
  }
});
...
function renderLogin(errorMsg) {
  const app = document.getElementById('app');
  app.innerHTML = `
    <div style="min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:calc(24px + env(safe-area-inset-top)) 24px calc(24px + env(safe-area-inset-bottom));text-align:center;background:${BLACK}">
      <div style="font-weight:900;font-size:30px;letter-spacing:-1px;text-transform:uppercase;margin-bottom:6px">
        <span style="color:${WHITE}">THE </span><span style="color:${MINT};text-shadow:0 0 30px ${MINT}55">SLAYERS</span>
      </div>
      <div style="color:${GREY};font-size:12px;margin-bottom:32px">v10.0</div>
      <div class="card" style="width:100%;max-width:300px;padding:24px;border-color:rgba(56,189,130,0.1)">
        ...
        <input id="codeInput" type="text" placeholder="SLAY-XXXXXX" autocapitalize="characters" autocomplete="off"
          style="width:100%;background:rgba(0,0,0,0.35);border:0.5px solid ${BORDER};border-radius:12px;padding:14px;color:${WHITE};font-size:16px;text-align:center;letter-spacing:2px;margin-bottom:14px;outline:none" class="mono"/>
...
async function attemptLogin() {
  ...
  try {
    ...
    if (res.status === 401) {
      let msg = 'That code is not valid.';
...
setInterval(() => { if (getCode()) fetchAll(); }, 60000);
