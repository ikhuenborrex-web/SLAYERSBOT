...
const TAG_EMOJIS = { Displacement: '', 'Liq Sweep': '🧹', MSS: '📐', 'Eng. Liq': '⚡', FVG: '💰', OB: '🏦', CRT: '📈', 'Early Entry': '⏰', Confident: '🏋️', Impatient: '🤯', Neutral: '😐' };
function tagStyle(tag) {
  const isFeeling = tag === 'Confident' || tag === 'Impatient' || tag === 'Neutral';
  const isEarly = tag === 'Early Entry';
  if (isEarly) return `background:${RED_DIM};color:${RED};border:0.5px solid ${RED}33`;
  if (isFeeling) return `background:${GOLD_DIM};color:${GOLD};border:0.5px solid ${GOLD}33`;
  return `background:${MINT_DIM};color:${MINT};border:0.5px solid ${MINT}1A`;
}

function calendarGrid(outcomes, label, getDate, getOutcome) {
  const now = new Date();
  ...
  for (let i = 0; i < 28; i++) {
    ...
    const match = outcomes.find(o => getDate(o) === ds);
    const outcome = match ? getOutcome(match) : null;
    const color = outcome === 'WIN' || outcome === 'TP1' || outcome === 'TP2' ? MINT
      : outcome === 'SL' ? RED
      : outcome === 'BE' ? GOLD
      : 'rgba(255,255,255,0.04)';
    cells.push(`<span style="display:inline-block;width:10px;height:10px;border-radius:2.5px;background:${color}"></span>`);
  }
  const dayHeaders = days.map(d => `<span style="font-size:7px;color:${GREY};width:10px;text-align:center;display:inline-block">${d}</span>`).join('');
  const rows = [];
  for (let w = 0; w < 4; w++) {
    rows.push(`<div style="display:flex;gap:3px;margin-top:3px">${cells.slice(w * 7, w * 7 + 7).join('')}</div>`);
  }
  return `<div class="card" style="animation-delay:0.05s">
    ...
    <div style="display:flex;gap:3px">${dayHeaders}</div>
    ${rows.join('')}
  </div>`;
}
...
function journalScreen() {
  ...
  return `
    ...
    <!-- Your Trades heatmap -->
    ${calendarGrid(entries, 'Your Trades', e => e.createdAt ? e.createdAt.slice(0, 10) : '', e => e.outcome)}

    <!-- Bot Trades heatmap -->
    ${calendarGrid(state.botHistory, 'Bot Trades', o => o.time ? o.time.slice(0, 10) : '', o => o.outcome)}
    ...
    ${filtered.length ? filtered.map((e, i) => `
      <div class="card" onclick="editEntry('${e.id}')" style="padding:12px 16px;cursor:pointer;animation-delay:${i*0.03}s">
        <div style="display:flex;gap:12px;align-items:flex-start">
          ...
          <div style="flex:1;min-width:0">
            ...
            ${e.tags && e.tags.length ? `<div style="display:flex;gap:4px;margin-top:5px">${e.tags.map(t => `<span style="font-size:8.5px;padding:2px 8px;border-radius:99px;${tagStyle(t)}">${TAG_EMOJIS[t] || ''} ${t}</span>`).join('')}</div>` : ''}
          </div>
        </div>
      </div>
...
function journalEntryForm() {
  const edit = state.editingEntry;
  const title = edit ? 'Edit Journal Entry' : 'New Journal Entry';
  const btnLabel = edit ? 'Update Entry' : 'Save Entry';
  const pairV = edit ? edit.pair || '' : '';
  const dirV = edit ? edit.direction || 'BUY' : 'BUY';
  const tfV = edit ? edit.tf || '4H' : '4H';
  const outV = edit ? edit.outcome || 'WIN' : 'WIN';
  const rV = edit ? edit.rMultiple || '' : '';
  const durV = edit ? edit.duration || '' : '';
  const notesV = edit ? edit.notes || '' : '';
  const tagOpts = ['Displacement', 'Liq Sweep', 'MSS', 'Eng. Liq', 'FVG', 'OB', 'CRT', 'Early Entry', 'Confident', 'Impatient', 'Neutral'];
  return `
    <div class="card" style="border-color:${MINT}33;animation-delay:0s">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px">
        ...
        <select id="jDir" style="background:${BLACK};border:0.5px solid ${BORDER};border-radius:8px;padding:10px;color:${WHITE};font-size:12px;outline:none">
          <option value="BUY" ${dirV === 'BUY' ? 'selected' : ''}>BUY</option><option value="SELL" ${dirV === 'SELL' ? 'selected' : ''}>SELL</option>
        </select>
        <select id="jTF" style="background:${BLACK};border:0.5px solid ${BORDER};border-radius:8px;padding:10px;color:${WHITE};font-size:12px;outline:none">
          <option value="1H" ${tfV === '1H' ? 'selected' : ''}>1H</option><option value="4H" ${tfV === '4H' ? 'selected' : ''}>4H</option><option value="DAILY" ${tfV === 'DAILY' ? 'selected' : ''}>DAILY</option>
        </select>
        <select id="jOutcome" style="background:${BLACK};border:0.5px solid ${BORDER};border-radius:8px;padding:10px;color:${WHITE};font-size:12px;outline:none">
          <option value="WIN" ${outV === 'WIN' ? 'selected' : ''}>WIN</option><option value="TP1" ${outV === 'TP1' ? 'selected' : ''}>TP1</option><option value="TP2" ${outV === 'TP2' ? 'selected' : ''}>TP2</option>
          <option value="SL" ${outV === 'SL' ? 'selected' : ''}>SL</option><option value="BE" ${outV === 'BE' ? 'selected' : ''}>BE</option>
        </select>
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;margin:10px 0">
        ${tagOpts.map(t => {
          const active = edit && edit.tags && edit.tags.includes(t);
          return `<span onclick="toggleTag(this, '${t}')" style="font-size:9px;padding:4px 10px;border-radius:99px;cursor:pointer;background:${active ? MINT_DIM : 'rgba(255,255,255,0.03)'};color:${active ? MINT : GREY};border:0.5px solid ${active ? MINT + '33' : 'rgba(255,255,255,0.05)'}">${t}</span>`;
        }).join('')}
      </div>
      ...
      ${edit ? `<div style="text-align:center;margin-top:10px"><span onclick="deleteEntry('${edit.id}')" style="font-size:12px;color:${RED};font-weight:600;cursor:pointer">Delete this entry</span></div>` : ''}
...
async function saveJournalEntry() {
  ...
  try {
    const body = { pair, direction, tf, outcome, rMultiple, duration, notes, tags: [...selectedTags] };
    const res = edit
      ? await fetch(withCode('/api/journal/' + edit.id), { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
      : await fetch(withCode('/api/journal'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    if (res.ok) {
...
window.editEntry = (id) => {
  state.editingEntry = state.journal.find(e => e.id === id) || null;
  if (state.editingEntry) selectedTags = [...(state.editingEntry.tags || [])];
  state.showEntryForm = false;
  render();
};
...
function render() {
  ...
  app.innerHTML = `
    <div style="display:flex;flex-direction:column;height:100dvh;background:${BLACK};color:${WHITE};position:relative">
...
function overviewScreen() {
  const trackedSignalIds = new Set(state.signals.filter(s => s.isTracked).map(s => s.id));
  const myActiveTrades = state.active.filter(t => trackedSignalIds.has(t.sigId));
  return `
    ${myActiveTrades.map(activeTradeWidget).join('')}
    <div class="section-h">Market Pulse</div>
    ${marketPulseRow()}
    ${statsOverview()}
    ${state.signals.length ? `<div class="section-h">Recent Signals</div>` + state.signals.map(signalCard).join('') : ''}
    ${!state.signals.length && !myActiveTrades.length ? emptyState(state.fetchError ? 'Connection problem: ' + state.fetchError : 'No signals yet. Waiting for the next scan...') : ''}
  `;
}
...
function marketPulseRow() {
  ...
  const chips = pairs.slice(0, 11).map(p => {
    const c = p.signalDir === 'BULLISH' ? MINT : p.signalDir === 'BEARISH' ? RED : TEXT2;
    const bg = p.signalDir !== 'NONE' ? (p.signalDir === 'BULLISH' ? MINT_DIM : RED_DIM) : 'rgba(255,255,255,0.02)';
