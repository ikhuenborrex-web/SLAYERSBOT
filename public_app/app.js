...
function journalScreen() {
  ...
  function renderCells(outcomes, getDate, getOutcome) {
    ...
    for (var i = 0; i < 28; i++) {
      ...
      cells += '<span style="display:inline-block;width:10px;height:10px;border-radius:2.5px;background:' + color + '"></span>';
    }
    var rows = '';
    for (var w = 0; w < 4; w++) {
      rows += '<div style="display:flex;gap:3px;margin-top:3px">' + cells.slice(w * 7, w * 7 + 7) + '</div>';
    }
    return rows;
  }
