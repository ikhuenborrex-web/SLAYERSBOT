...
function signalCard(s){
  ...
  return '<div class="card" onclick="openDetail(\''+s.id+'\')" style="cursor:pointer;border-left:2.5px solid '+tierColor+';animation-delay:'+(Math.random()*0.2)+'s">'+
    ...
    (s.isEarly?'<span style="font-size:8px;color:'+C.orange+';background:'+C.orangeSoft+';border:0.5px solid '+C.orange+'44;border-radius:4px;padding:2px 8px;font-weight:600;margin-left:6px">\u26A1 EARLY</span>':'')+
    '<span style="margin-left:auto;font-size:10px;color:'+C.text2+'">'+timeAgo(s.time)+'</span>'+
    '</div>'+
    (s.earlyEntry?'<div style="font-size:10px;color:'+C.orange+';font-weight:600;margin-bottom:6px">\u26A1 Dual Entry: Aggressive + Conservative</div>':'')+
    (s.conservativePending?'<div style="font-size:10px;color:'+C.text2+';font-weight:500;margin-bottom:6px">\u23F3 Conservative QM entry pending at '+fmt(s.conservativeQmLevel)+'</div>':'')+
    ...
    '</div>'+
    (s.earlyEntry?'<div style="display:flex;gap:12px;font-size:12px;color:'+C.text2+';margin-top:4px;padding-top:6px;border-top:0.5px solid rgba(255,255,255,0.06)">'+
    '<span><span style="color:'+C.orange+'">\u26A1 Early</span> <span style="font-weight:700;color:'+C.orange+'">'+fmt(s.earlyEntry)+'</span></span>'+
    '<span><span style="color:'+C.orange+'">SL</span> <span style="font-weight:700;color:'+C.red+'">'+fmt(s.earlySl)+'</span></span>'+
    (s.earlyTp1?'<span><span style="color:'+C.orange+'">TP1</span> <span style="font-weight:700;color:'+C.lime+'">'+fmt(s.earlyTp1)+'</span></span>':'')+
    '</div>':'')+
...
function detailPage(s){
  ...
  return '<div style="display:flex;flex-direction:column;height:100dvh;background:'+C.bg+';color:'+C.white+'">'+
    ...
    (s.earlyEntry?'<div style="display:flex;justify-content:space-between;padding:9px 0"><span style="color:'+C.orange+'">'+icon('bolt',C.orange,13)+' Aggressive Entry (sweep)</span>'+mono(fmt(s.earlyEntry),C.orange,16)+'</div>':'')+
    '<div style="display:flex;justify-content:space-between;padding:9px 0"><span style="color:'+C.text2+'">'+(s.refinedEntry?'4H Zone':'Conservative Entry (QM)')+'</span>'+mono(fmt(s.entry),C.white,16)+'</div>'+
    ...
    (s.earlyEntry?'<div style="padding:9px 0 0;margin-top:6px;border-top:0.5px solid rgba(255,255,255,0.06);font-size:11px;color:'+C.orange+'">\u26A1 Aggressive: '+fmt(s.earlyEntry)+' | SL: '+fmt(s.earlySl)+' | TP: '+fmt(s.earlyTp1||'')+'</div>':'')+
    '</div>'+
...
window.copyTrade=function(id){for(var i=0;i<state.signals.length;i++){var s=state.signals[i];if(s.id===id){var txt=(s.type==='BULLISH'?'BUY':'SELL')+' '+s.pair+' | Entry: '+fmt(s.entry)+' | SL: '+fmt(s.sl)+(s.tp1?' | TP1: '+fmt(s.tp1):'')+(s.tp2?' | TP2: '+fmt(s.tp2):'')+' | '+s.system+' Signal';navigator.clipboard.writeText(txt).then(function(){showToast('Trade copied to clipboard');}).catch(function(){});break;}}};
