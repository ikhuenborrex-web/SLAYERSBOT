...
function render() {
  ...
  if (state.selected) {
    app.innerHTML = `<div style="display:flex;flex-direction:column;height:100dvh;background:${BLACK}"><div style="flex:1;overflow-y:auto;background:${BLACK}">${detailPage(state.selected)}</div></div>`;
    return;
  }
  app.innerHTML = `<div style="display:flex;flex-direction:column;height:100dvh;background:${BLACK}">
      ...
      <div style="position:relative;display:flex;border-top:1px solid ${BORDER};background:rgba(13,22,17,0.6);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);padding-bottom:env(safe-area-inset-bottom);flex-shrink:0">
