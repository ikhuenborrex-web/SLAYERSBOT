...
function signalCard(s){
  ...
  return '<div class="card" onclick="openDetail(\''+s.id+'\')" style="cursor:pointer;border-left:2.5px solid '+(isDual?C.orange:tierColor)+';animation-delay:'+(Math.random()*0.2)+'s">'+
    ...
    '</div>'+
    // Dual entry: pending conservative banner
    (isDual&&!s.consEntry?'<div style="font-size:10px;color:'+C.orange+';background:'+C.orangeSoft+';border:0.5px solid '+C.orange+'44;border-radius:6px;padding:5px 8px;font-weight:600;margin-bottom:8px">\u23F3 Conservative QM entry pending at '+fmt(s.qmLevel)+'</div>':'')+
