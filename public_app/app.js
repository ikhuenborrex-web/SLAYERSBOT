...
// ===== MINI PRICE BAR FOR SIGNAL CARDS =====
function priceBar(s) {
  if (!s.entry || !s.sl || !s.tp1) return '';
  var entry = parseFloat(s.entry);
  var sl = parseFloat(s.sl);
  var tp1 = parseFloat(s.tp1);
  if (isNaN(entry) || isNaN(sl) || isNaN(tp1)) return '';
  var isBuy = s.type === 'BULLISH' || s.type === 'BUY';
  var min = isBuy ? sl : tp1;
  var max = isBuy ? tp1 : sl;
  var range = max - min;
  if (range <= 0) return '';
  var ePct = ((entry - min) / range * 100).toFixed(1);
  var sPct = ((sl - min) / range * 100).toFixed(1);
  var tPct = ((tp1 - min) / range * 100).toFixed(1);
  // Track background: left to right
  var barBg = PANEL;
  var fillLeft = Math.min(ePct, sPct);
  var fillW1 = Math.abs(ePct - sPct);
  var fillW2 = Math.abs(ePct - tPct);
  var c1 = isBuy ? RED_DIM : MINT_DIM;
  var c2 = isBuy ? MINT_DIM : RED_DIM;
   return '<div style="position:relative;height:30px;margin:8px 0 0">' +
     '<div style="position:absolute;top:10px;left:0;right:0;height:4px;background:' + barBg + ';border-radius:99px">' +
     '<div style="position:absolute;top:0;left:' + fillLeft + '%;width:' + fillW1 + '%;height:100%;background:' + c1 + ';border-radius:99px"></div>' +
     '<div style="position:absolute;top:0;left:' + Math.min(ePct, tPct) + '%;width:' + fillW2 + '%;height:100%;background:' + c2 + ';border-radius:99px"></div>' +
     '</div>' +
     '<div style="position:absolute;top:4px;left:' + sPct + '%;width:10px;height:10px;border-radius:50%;background:' + RED + ';border:1.5px solid ' + BLACK + ';transform:translateX(-50%);z-index:2" title="SL"></div>' +
     '<div style="position:absolute;top:4px;left:' + ePct + '%;width:10px;height:10px;border-radius:50%;background:' + MINT + ';border:1.5px solid ' + BLACK + ';transform:translateX(-50%);z-index:3" title="Entry"></div>' +
     '<div style="position:absolute;top:4px;left:' + tPct + '%;width:10px;height:10px;border-radius:50%;background:' + GOLD + ';border:1.5px solid ' + BLACK + ';transform:translateX(-50%);z-index:2" title="TP1"></div>' +
     '<div style="position:absolute;top:20px;left:' + sPct + '%;transform:translateX(-50%);font-size:7.5px;color:' + RED + ';font-weight:600;white-space:nowrap">' + fmt(sl) + '</div>' +
     '<div style="position:absolute;top:20px;left:' + ePct + '%;transform:translateX(-50%);font-size:7.5px;color:' + MINT + ';font-weight:600;white-space:nowrap">' + fmt(entry) + '</div>' +
     '<div style="position:absolute;top:20px;left:' + tPct + '%;transform:translateX(-50%);font-size:7.5px;color:' + GOLD + ';font-weight:600;white-space:nowrap">' + fmt(tp1) + '</div></div>';
}
...
function journalScreen() {
  ...
  function entryHtml(e) {
    ...
    return '<div class="card" onclick="editEntry(\'' + e.id + '\')" style="padding:12px 16px;cursor:pointer;animation-delay:0s">' +
      ...
      notesHtml + tagsHtml + '</div></div></div>';
  }
  ...
  return '<div class="section-h">Journal</div>' +
    ...
    '<button onclick="window.open(withCode(\'/api/journal\'), \'_blank\')" class="btn-outline" style="flex:0">Export</button></div>' +
...
function overviewScreen() {
  ...
  if (state.signals.length) {
    signalsHtml = '<div class="section-h">Recent Signals</div>';
    for (var i = 0; i < state.signals.length; i++) { signalsHtml += signalCard(state.signals[i]); }
  }
  var emptyHtml = '';
  if (!state.signals.length && !myActive.length) {
